	...
	function self.DischargeGun()
		...
		if rayL > -1 then
			...
			for i = 0, 1 + self.chargenum do
				...
				pulseexplosion.Vel = Vector(115 + 2 * self.chargenum, 0):RadRotate(angle)
				...
				pulseexplosion.Pos = hitpos
				pulseexplosion.Team = self.Team
				pulseexplosion.Vel = Vector(110 + 3 * self.chargenum, 0):RadRotate(angle)
				pulseexplosion.Mass = pulseexplosion.Mass + self.chargenum * 0.015
				pulseexplosion.IgnoresTeamHits = true
				MovableMan:AddParticle(pulseexplosion)
			end
			PrimitiveMan:DrawLinePrimitive(self.MuzzlePos, self.MuzzlePos + Range, 48, 1 + self.chargenum * 1.2)
	...
	if parent and IsAHuman(parent) then
		...
		if self.Magazine then
			...
			if self:IsActivated() then
				...
				if self.firetimer:IsPastSimMS(255) and self.chargenum > 0 then
					self.DischargeGun()
				end
			end
		end
