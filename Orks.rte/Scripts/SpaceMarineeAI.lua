...
function Create(self)
	...
	self.suppressedVoicelineDelay = 2000;
	
	self.healthUpdateTimer = Timer();
	self.oldHealth = self.Health;

	self.spotVoiceLineTimer = Timer();
	self.spotVoiceLineDelay = 10000;
...
function Update(self)
	...
	if (self:IsDead() ~= true) then
		...
		osog.handleHealth(self);

		osog.handleAITargetLogic(self);

		osog.handleSuppression(self);
		
	else
	
		osog.handleDying(self);
