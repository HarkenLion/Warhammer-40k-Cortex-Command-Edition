function Create(self)
	...
	self.equipSound = CreateSoundContainer("CombatAxe Equip", "imperialguard.rte")
	self.swingSound = CreateSoundContainer("CombatAxe Swing", "imperialguard.rte")
	self.bounceSound = CreateSoundContainer("CombatAxe Bounce", "imperialguard.rte")
			...
			if self.connect == true or self.num > math.pi * 1.5 then
				...
				if moid ~= 255 and moid ~= 0 then
					for i = 0, 2 do
						...
						part.Vel = parent.Vel + Vector((85 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
						part.Mass = 0.6 * RangeRand(0.75, 1.25)
						part.Sharpness = 12 * RangeRand(0.75, 1.25)
						part.Team = self.Team
						part.IgnoresTeamHits = true
