...
function NativeSentinelAI:Create(Owner)
	local Members = {}
...
function NativeSentinelAI:Update(Owner)
	...
	if self.isBrain then
		FoundMO, HitPoint = HumanBehaviors.CheckEnemyLOS(self, Owner)
	else
		FoundMO, HitPoint = HumanBehaviors.LookForTargets(self, Owner)
	end
	...
	--if self.jump and Owner.Jetpack.JetTimeLeft > TimerMan.DeltaTimeMS then
	...
	--		if Owner.Jetpack.JetTimeLeft >= self.minBurstTime then	-- don't start the jetpack unless there is enough fuel to fire a burst that is long enough to be useful
...
function NativeSentinelAI:CreateSentryBehavior(Owner)
...
function NativeSentinelAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()
...
function NativeSentinelAI:CreateHtHBehavior(Owner)
...
function NativeSentinelAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
...
function NativeSentinelAI:CreateMoveAroundBehavior(Owner)
...
function NativeSentinelAI:CreateFaceAlarmBehavior(Owner)
