...
function NecronBehaviors.LookForTargets(AI, Owner)
	local Origin
	local viewAng
...
function NecronBehaviors.CheckEnemyLOS(AI, Owner)
	if not AI.Enemies then -- add all enemy actors on our screen to a table and check LOS to them, one per frame
		...
		return NecronBehaviors.LookForTargets(AI, Owner) -- cast rays like normal actors occasionally
	else
			...
			if MovableMan:ValidMO(Enemy) then
				...
				if Enemy.ClassName == "ADoor" then
					...
					if Door and Door:IsAttached() then
						...
						return NecronBehaviors.LookForTargets(AI, Owner) -- this door is destroyed, cast rays like normal actors
					end
				else
...
function NecronBehaviors.CalculateThreatLevel(MO, Owner)
...
function NecronBehaviors.ProcessAlarmEvent(AI, Owner)
	AI.AlarmPos = nil
	...
	for Event in MovableMan.AlarmEvents do
		if Event.Team ~= Owner.Team then -- caused by some other team's activites - alarming!
			loudness = 30 + FrameMan.PlayerScreenWidth * 0.5 * Owner.Perceptiveness * (Event.Range / 500) -- adjust the audiable range to the screen resolution
...
function NecronBehaviors.GetGrenadeAngle(AimPoint, TargetVel, StartPos, muzVel)
...
function NecronBehaviors.EquipPrimaryWeapon(AI, Owner)
	if Owner.EquippedItem then
		if
...
function NecronBehaviors.Sentry(AI, Owner)
	local sweepUp = true
	...
	if NecronBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
				if not NecronBehaviors.EquipPrimaryWeapon(AI, Owner) then
...
function NecronBehaviors.Patrol(AI, Owner)
...
function NecronBehaviors.GoldDig(AI, Owner)
...
function NecronBehaviors.BrainSearch(AI, Owner)
	if NecronBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
...
function NecronBehaviors.WeaponSearch(AI, Owner)
...
function NecronBehaviors.ToolSearch(AI, Owner)
	local range, minDist, HD
	local Devices = {}
...
function NecronBehaviors.GoToWpt(AI, Owner)
...
function NecronBehaviors.GoProne(AI, Owner, TargetPos, targetID)
	...
	then
		...
		if
			SceneMan:CastObstacleRay(PronePos, Dist, Vector(), Vector(), 0, Owner.IgnoresWhichTeam, rte.grassID, 1)
			> -1
		then
			return false
		end
...
function NecronBehaviors.GetProjectileData(Owner)
...
function NecronBehaviors.ShootTarget(AI, Owner)
	...
	if not AI.flying and AI.Target.Vel.Largest < 4 and NecronBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID) then
	...
	while true do
		...
		if Owner.FirearmIsReady then
			...
			if not PrjDat or PrjDat.MagazineName ~= Weapon.Magazine.PresetName then
				PrjDat = NecronBehaviors.GetProjectileData(Owner)
				...
				if checkAim then
					...
					elseif range < PrjDat.rng then
						...
						aimTarget = NecronBehaviors.GetAngleToHit(PrjDat, Dist)
						if aimTarget then
							AI.canHitTarget = true
						else
							AI.canHitTarget = false
...
function NecronBehaviors.ThrowTarget(AI, Owner)
		...
		if scan < 1 then
			...
			if LOS then -- don't sharp aim until LOS has been confirmed
				if Owner.ThrowableIsReady then
					aim = NecronBehaviors.GetGrenadeAngle(
						AimPoint,
...
function NecronBehaviors.AttackTarget(AI, Owner)
...
function NecronBehaviors.MoveAroundActor(AI, Owner)
...
function NecronBehaviors.GetAngleToHit(PrjDat, Dist)
...
function NecronBehaviors.ShootArea(AI, Owner)
	...
	if
		...
		and NecronBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID)
	then
		aimTime = aimTime + 500
	end
...
function NecronBehaviors.FaceAlarm(AI, Owner)
...
local Proxy = {}
local Mt = {
	__index = NecronBehaviors,
	__newindex = function(Table, k, v)
		error("The NecronBehaviors table is read-only.", 2)
	end,
}
setmetatable(Proxy, Mt)
NecronBehaviors = Proxy
