...
function NativeNecronAI:Create(Owner)
	local Members = {}
...
function NativeNecronAI:Update(Owner)
...
function NativeNecronAI:CreateSentryBehavior(Owner)
...
function NativeNecronAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()

	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(NecronBehaviors.ShootTarget)
		self.NextBehaviorName = "ShootTarget"
	elseif Owner:EquipThrowable(true) then
		self.NextBehavior = coroutine.create(NecronBehaviors.ThrowTarget)
...
function NativeNecronAI:CreateHtHBehavior(Owner)
...
function NativeNecronAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
...
function NativeNecronAI:CreateMoveAroundBehavior(Owner)
...
function NativeNecronAI:CreateFaceAlarmBehavior(Owner)
