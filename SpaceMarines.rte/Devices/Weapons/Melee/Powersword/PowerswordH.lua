function Create(self)
	...
	self.critsMax = 20;	-- max amount of critical hits per swing

	self.speed = 0.5;	-- variable(not)
	self.equipSound = CreateSoundContainer("Powersword Equip", "SpaceMarines.rte");
	self.swingSound = CreateSoundContainer("HammahSwing", "SpaceMarines.rte");
	...
	if parent and IsAHuman(parent) then
		...
		elseif self.swing == true then
			...
			if self.connect == true or self.num > math.pi * 1.5 then
				...
				local tracevec = parent.Vel + Vector((62 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
				local moid = SceneMan:CastMORay(self.MuzzlePos, tracevec, self.ID, self.Team, 0, false, 3)
				
				local part = CreateMOPixel("Particle Powersword 4");
				...
				part.Sharpness = 650 * RangeRand(0.75, 1.25);
				...
				if moid ~= 255 and moid ~= 0 then
					for i = 0, 1 do
						...
						local part2 = CreateAEmitter("Las BoomHA");
						part2.Pos = SceneMan:GetLastRayHitPos()
						part2.Vel = parent.Vel + Vector((100 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3);
						part2.Mass = 0.1 * RangeRand(0.75, 1.25);
						part2.Team = self.Team;
						part2.IgnoresTeamHits = true;
