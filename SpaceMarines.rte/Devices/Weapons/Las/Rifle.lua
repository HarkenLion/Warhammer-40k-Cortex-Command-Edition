...
function OnFire(self)
	...
	rayL = SceneMan:CastObstacleRay(Vector(self.MuzzlePos.X,self.MuzzlePos.Y),vect,vect,vect,self.ID,self.Team,0,3);  
...
--WEAPON DISCHARGE
	...
	if rayL>-1 then  
		...
		local pulseexplosion = CreateAEmitter("Las Boom");
		pulseexplosion.Pos = hitpos
		pulseexplosion.Team = self.Team;
		...
		PrimitiveMan:DrawLinePrimitive(self.MuzzlePos,hitpos,198,4)
		...
		PrimitiveMan:DrawLinePrimitive(self.MuzzlePos,self.MuzzlePos+self:RotateOffset(Vector(1900,0)),198,4)
	end  
