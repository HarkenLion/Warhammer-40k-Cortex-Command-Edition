	...
	if self:IsActivated() and not self:IsReloading() then
		if self.Magazine.RoundCount > 0 then
			...
			if self.fireTimer:IsPastSimMS(20) then
				...
				local effectPar = CreateMOPixel("Space Marine Melta Gun Effect Particle "..math.ceil(math.random()*4));
				...
				for i = 1, beamEffectAmount do
					local effectPar = CreateMOPixel("Particle Space Marine Melta Gun Beam Effect ".. math.ceil((((i/beamEffectAmount)*rayLengthB)/465)*20) );
					local hitPos = self.MuzzlePos + Vector((i/beamEffectAmount)*rayLengthB*self.negatore,0):RadRotate(self.RotAngle)
					if SceneMan.SceneWrapsX == true then
