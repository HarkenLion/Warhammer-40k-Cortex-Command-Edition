function Create(self)
	...
	self.ejectSound = CreateSoundContainer("Tank Eject", "imperialguard.rte");
end
...
function OnFire(self)
	...
	if MovableMan:ValidMO(self.operator) then
		self.operator:AddAbsForce(Vector(-40500*self.operator.FlipFactor,0):RadRotate(self.RotAngle),Vector(-10*self.operator.FlipFactor,-30):RadRotate(self.operator.RotAngle))
	end
end
		...
		if self.fireTimer:IsPastSimMS(205) then
			...
			if self.shellejected == false and self.fireTimer:IsPastSimMS(650) then
				if self.operator then
					...
					cannoncasing.Pos = self.operator.Pos+Vector(-5*self.operator.FlipFactor,-45):RadRotate(self.operator.RotAngle)
