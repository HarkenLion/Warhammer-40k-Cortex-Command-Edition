...
function Update(self)
	...
	local i = 0;
	while i < 4 do
		local e = CreateMOPixel("Plas Shot glow");
		e.Vel = self.Vel;
	...
	if self.ToDelete == true then
		...
		if rand > 2 then e = CreateMOSParticle("Plasma Burst B","SpaceMarines.rte") end
		if rand < -2 then e = CreateMOSParticle("Plasma Burst C","SpaceMarines.rte") end
