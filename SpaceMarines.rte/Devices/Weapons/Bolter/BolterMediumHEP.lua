...
function Update(self)
	if self.ToDelete == true or self.ToSettle == true then
		local e = CreateAEmitter("Bolter Primaris Shot")
		local rand = math.random(-4,4)
		if rand > 2 then e = CreateAEmitter("Bolter Primaris Shot") end
		if rand > 3 then e = CreateAEmitter("Bolter Primaris Shot") end
		if rand < -2 then e = CreateAEmitter("Bolter Primaris Shot") end
		if rand < -3 then e = CreateAEmitter("Bolter Primaris Shot") end
		e.Pos =  self.Pos
		...
		local e = CreateAEmitter("Bolter Primaris Shot","SpaceMarines.rte"); --CreateMOSParticle("Small Smoke Ball 1");
