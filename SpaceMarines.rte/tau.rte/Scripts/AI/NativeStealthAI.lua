...
function NativeStealthAI:Create(Owner)
	local Members = {}
...
function NativeStealthAI:Update(Owner)
			...
			if FoundMO.Team == Owner.Team then -- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if
							TauBehaviors.CalculateThreatLevel(FoundMO, Owner)
							> TauBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
						then
...
function NativeStealthAI:CreateSentryBehavior(Owner)
...
function NativeStealthAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()
...
function NativeStealthAI:CreateHtHBehavior(Owner)
...
function NativeStealthAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
...
function NativeStealthAI:CreateMoveAroundBehavior(Owner)
...
function NativeStealthAI:CreateFaceAlarmBehavior(Owner)
