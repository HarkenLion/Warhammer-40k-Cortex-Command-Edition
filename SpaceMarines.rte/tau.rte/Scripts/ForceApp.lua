...
function Create(self)
	...
	self.triggerang = math.pi / 4 --4
	self.triggerangBG = math.pi / 3 --4
	self.musclemult = 1 -- 0.5 --2 --4 --1.5; --1.25 --2.5 --1.5
...
function Update(self)
	...
	if self.c:IsState(3) and not self.c:IsState(5) then
		...
		if self.FlipFactor == 1 then
			--FORWARD LEG PULL
			if MovableMan:ValidMO(self.FGLeg) and MovableMan:ValidMO(self.FGFoot) and self.FGLeg.RotAngle < self.triggerang then
			...
			if MovableMan:ValidMO(self.BGLeg) and MovableMan:ValidMO(self.BGFoot) and self.BGLeg.RotAngle < self.triggerang then
			...
			--BACK LEG PUSH
			if MovableMan:ValidMO(self.FGLeg) and MovableMan:ValidMO(self.FGFoot) and self.FGLeg.RotAngle > self.triggerangBG then
			...
			if MovableMan:ValidMO(self.BGLeg) and MovableMan:ValidMO(self.BGFoot) and self.BGLeg.RotAngle > self.triggerangBG then
	...
	elseif self.c:IsState(4) and not self.c:IsState(5) then
		...
		if self.FlipFactor == -1 then
			...
			MovableMan:ValidMO(self.FGLeg)
				and MovableMan:ValidMO(self.FGFoot)
			...
			MovableMan:ValidMO(self.BGLeg)
				and MovableMan:ValidMO(self.BGFoot)
	...
	if MovableMan:ValidMO(self.FGFoot) then
		self.FGFootPrevPos = self.FGFoot.Pos.X - self.Pos.X
	end
	if MovableMan:ValidMO(self.BGFoot) then
		self.BGFootPrevPos = self.BGFoot.Pos.X - self.Pos.X
	end
