...
function Create(self)
	...
	self.setalt = 7;	
	self.moveSound = CreateSoundContainer("Tank Move", "ImperialGuard.rte");
	self.idleSound = CreateSoundContainer("Tank Idle", "ImperialGuard.rte");
	
	for attachable in self.Attachables do
		if attachable.PresetName == "Rhino Tank Tread UM"" then
...
function Update(self)
	if MovableMan:ValidMO(self) then
		...
		if self.Vel.Y > -1 then
			...
			local groundray = SceneMan:CastStrengthRay(self.Pos + Vector(-28*self.FlipFactor,3):RadRotate(self.RotAngle),Vector(55*self.FlipFactor,3):RadRotate(self.RotAngle),0,terrcheck,1,0,true);
			local groundray2 = SceneMan:CastStrengthRay(self.Pos + Vector(-85*self.FlipFactor,0):RadRotate(self.RotAngle),Vector(55*self.FlipFactor,3):RadRotate(self.RotAngle),0,terrcheck,1,0,true);
			local groundray3 = SceneMan:CastStrengthRay(self.Pos + Vector(28*self.FlipFactor,3):RadRotate(self.RotAngle),Vector(-55*self.FlipFactor,3):RadRotate(self.RotAngle),0,terrcheck,1,0,true);
			local groundray4 = SceneMan:CastStrengthRay(self.Pos + Vector(55*self.FlipFactor,0):RadRotate(self.RotAngle),Vector(-55*self.FlipFactor,3):RadRotate(self.RotAngle),0,terrcheck,1,0,true);
