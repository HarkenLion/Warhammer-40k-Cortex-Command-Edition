...
require("Scripts/terminatorAICaptainBehaviourss")
...
function Create(self)
	...
	if math.random(0, 1) == 0 then
		self.voiceSounds = {
		Death = CreateSoundContainer("VOO Death VariantOne", "SpaceMarines.rte"),
		Spot = CreateSoundContainer("VOO Spot VariantOne", "SpaceMarines.rte"),
		suppressedLow = CreateSoundContainer("VOO Suppressed termie", "SpaceMarines.rte"),
		suppressedMedium = CreateSoundContainer("VOO Suppressed termieM", "SpaceMarines.rte"),
		suppressedHigh = CreateSoundContainer("VOO Suppressed termieH", "SpaceMarines.rte"),};
	else
		self.voiceSounds = {
		Death = CreateSoundContainer("VOO Death VariantOne", "SpaceMarines.rte"),
		Spot = CreateSoundContainer("VOO Spot VariantOne", "SpaceMarines.rte"),
	...
	AudioMan:PlayMusic("SpaceMarines.rte/Actors/Shared/boss theme termie.ogg", 2, -1.0);
...
function Update(self)
	...
	if (self:IsDead() ~= true) then
		...
		taibehaviours.handleHealth(self);

		taibehaviours.handleAITargetLogic(self);

		taibehaviours.handleSuppression(self);
		
	else
	
		taibehaviours.handleDying(self);
