...
package.path = package.path .. ";Mods/SpaceMarines.rte/?.lua";
require("SpaceMarines.rte/AI/MarineCAIBehaviourss")
...
function Create(self)
	...
	self.RTE = "SpaceMarines.rte";
	self.baseRTE = "SpaceMarines.rte";

	if math.random(0, 1) == 0 then
		self.voiceSounds = {
		Death = CreateSoundContainer("VO Death VariantOne", "SpaceMarines.rte"),
		Spot = CreateSoundContainer("VO Spot VariantOne", "SpaceMarines.rte"),
		suppressedLow = CreateSoundContainer("VOMARINES Suppressed spacemarinesH", "SpaceMarines.rte"),
		suppressedMedium = CreateSoundContainer("VOMARINES Suppressed spacemarinesM", "SpaceMarines.rte"),
		suppressedHigh = CreateSoundContainer("VOMARINES Suppressed spacemarinesH", "SpaceMarines.rte"),};
	else
		self.voiceSounds = {
		Death = CreateSoundContainer("VO Death VariantOne", "SpaceMarines.rte"),
		Spot = CreateSoundContainer("VO Spot VariantOne", "SpaceMarines.rte"),
...
function Update(self)
	...
	if (self:IsDead() ~= true) then
		...
		CHAPLAINAI.handleHealth(self);

		CHAPLAINAI.handleAITargetLogic(self);

		CHAPLAINAI.handleSuppression(self);
		
	else
	
		CHAPLAINAI.handleDying(self);
