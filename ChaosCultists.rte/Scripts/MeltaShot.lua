...
function Update(self)
	...
	while i < 3 do
		...
		if rand > 3 then
			e = CreateMOPixel("Melta Shot Glow B", "spacemarines.rte")
		end
		if rand > 2 then
			e = CreateMOPixel("Melta Shot Glow C", "spacemarines.rte")
		end
		if rand < -2 then
			e = CreateMOPixel("Melta Shot Glow D", "spacemarines.rte")
		end
		if rand < -3 then
			e = CreateMOPixel("Melta Shot Glow E", "spacemarines.rte")
		end
		e.Vel = self.Vel
	...
	if self.ToDelete == true then
		...
		local rand = math.random(-4, 4)
		if rand > 2 then
			e = CreateMOSParticle("Melta Burst B", "spacemarines.rte")
		end
		if rand < -2 then
			e = CreateMOSParticle("Melta Burst C", "spacemarines.rte")
		end
