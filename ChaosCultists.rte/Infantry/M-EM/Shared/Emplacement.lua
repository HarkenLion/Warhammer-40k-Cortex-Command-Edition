		...
		if self.motorParent and self.motorParent:IsAttached() == true then
			...
			elseif self.Magazine then
				if self.setAmmo == false then
					self.Magazine.RoundCount = self:NumberValueExists("Ammo Count")
							and self:GetNumberValue("Ammo Count")
						or 1
					self.setAmmo = true
				end
			end
					...
					then
						for actor in MovableMan.Actors do
							if
								...
								and SceneMan:ShortestDistance(actor.Pos, self.Pos, SceneMan.SceneWrapsX).Magnitude < 45
									...
									then --F to mount
										...
										then
											self.gunner:AddInventoryItem(
												CreateHDFirearm("Gunner Holder", "GrandGuard.rte")
											)
											self.gunner:EquipNamedDevice("Gunner Holder", true)
												...
												if IsAHuman(actor) then
													...
													then --F to mount
														...
														if
															self.spotter.FGArm
															and (
																not self.spotter.EquippedItem
																or (
																	self.spotter.EquippedItem
																	and self.spotter.EquippedItem.PresetName
																		~= "Gunner Holder"
																)
															)
														then
			...
			if self.crewSize > 2 then
				local spotterFrame = (math.abs(self.parent.AngularVel) > 7 or math.abs(self.parent.RotAngle) > 0.8)
						and 5
					or self.spotter and (self.parent.Team + 1)
					or 0
