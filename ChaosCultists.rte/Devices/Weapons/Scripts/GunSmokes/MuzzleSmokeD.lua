...
----------------------------- This one belongs to slower rifles -----------------------------------------------
----------------------------------------------------------------------------------------------------------------

function Create(self)
	self.barrelHeat = 0
	self.barrelHeatPerBullet = 7.5
	self.barrelHeatCooling = 0.75
	self.barrelHeatTop = 30
	self.heatTimer = Timer()
	self.heatDelayHeated = 240
	self.heatDelayUnderheat = 120
	self.heatDelay = 80

	self.barrelDarkerSmokeThresold = 10
	self.barrelSmokeHalver = 1
	self.barrelSmokeHalverValue = 0.2

	self.muzzleSmokeTimer = Timer()
	self.muzzleSmokeDelay = 250
	self.muzzleDarkerSmokeDelay = 200
	self.muzzleSmokeChance = 0.0025
	self.muzzleDarkerSmokeChance = 0.0035
end
