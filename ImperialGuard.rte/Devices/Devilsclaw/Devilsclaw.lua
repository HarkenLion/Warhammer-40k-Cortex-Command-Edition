function Create(self)
	...
	self.equipSound = CreateSoundContainer("Devilsclaw Equip", "ImperialGuard.rte")
	self.swingSound = CreateSoundContainer("Devilsclaw Swing", "ImperialGuard.rte")
	self.bounceSound = CreateSoundContainer("Devilsclaw Bounce", "ImperialGuard.rte")
...
function Update(self)
	...
	if parent and IsAHuman(parent) then
		...
		if self.cooldown == true then
			...
			local recovertime = math.max(0, 250 - 15 * parent.Sharpness)
		...
		elseif self.swing == true then
			...
			if self.connect == true or self.num > math.pi * 1.5 then
				...
				local tracevec = parent.Vel + Vector((35 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
				local moid = SceneMan:CastMORay(self.MuzzlePos, tracevec, self.ID, self.Team, 0, false, 3)

				local part = CreateMOPixel("Particle Devilsclaw 1")
				...
				if moid ~= 255 and moid ~= 0 then
					...
					local ustren = 40 + parent.Sharpness * 5 --90 --100
		...
		if self.windup ~= true and self.swing ~= true then
			offset = Vector(2, 3):RadRotate(-0.25 + math.sin(self.num) * 1.4)
