...
function OnFire(self)
	...
	if recoil2 < 0.024 then
		...
		self.recoil = recoil2 + 0.0010 + (4 - actor.Sharpness) * 0.65 * 0.0005
	...
	if rayL > -1 then
		...
		local pulseexplosion2 = CreateMOPixel("Rifle Las Particle Hit")
		pulseexplosion2.Pos = hitpos
		pulseexplosion2.Team = self.Team
		pulseexplosion2.Vel = Vector(125, 0):RadRotate(angle)
		...
		PrimitiveMan:DrawLinePrimitive(self.MuzzlePos, hitpos, 254, 1)

		local smoke = CreateMOPixel("Rifle Las Contact Flash Longlas")
		...
		PrimitiveMan:DrawLinePrimitive(self.MuzzlePos, self.MuzzlePos + self:RotateOffset(Vector(2500, 0)), 13, 1.5)
	end
