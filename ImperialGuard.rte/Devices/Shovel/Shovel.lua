...
function OnAttach(self, newParent)
	...
	if IsAHuman(rootParent) and MovableMan:IsActor(rootParent) then
		...
		humanRootParent.PieMenu:GetFirstPieSliceByPresetName("IG Shovel Fill Sandbag PieSlice").Enabled =
...
function Update(self)
	...
	if parent and IsActor(parent) then
		...
		local resource = parent:GetNumberValue("igshovelresource")
		if
			parent:IsPlayerControlled()
			and controller:IsState(Controller.AIM_SHARP)
			and (not controller:IsMouseControlled() or controller.AnalogAim:MagnitudeIsGreaterThan(0.9))
		then
			self.InheritedRotAngleOffset = math.min(
				self.InheritedRotAngleOffset + (0.0003 * self.RateOfFire),
				self.angleSize * 2
			)
		else
		...
		if self.FiredFrame then
			...
			if moCheck ~= rte.NoMOID then
				...
				for i = 1, particleCount do
					...
					damagePar.Vel = Vector(fireVec.X, fireVec.Y):RadRotate(
						(-spread * 2 + i * spread - self.InheritedRotAngleOffset * 1.4) * self.FlipFactor
					)
			...
			elseif not overhead and resource < 10 then
				...
				for i = 1, rayCount do
					...
					if terrRay then
						local material =
							SceneMan:GetMaterialFromID(SceneMan:GetTerrMatter(hitPos.X, hitPos.Y)).PresetName
						for _, terrainMaterial in pairs(self.suitableMaterials) do
							if material == terrainMaterial then
								...
								if hits > rayCount * 0.5 then
									...
									parent.PieMenu:GetFirstPieSliceByPresetName("IG Shovel Fill Sandbag PieSlice").Enabled = resource
										>= 10
