function Create(self)
	...
	self.equipSound = CreateSoundContainer("Powersword Equip", "ImperialGuard.rte")
	self.swingSound = CreateSoundContainer("Powersword Swing", "ImperialGuard.rte")
	self.bounceSound = CreateSoundContainer("Powersword Bounce", "ImperialGuard.rte")
...
function Update(self)
	...
	if parent and IsAHuman(parent) then
		...
		if self.cooldown == true then
			...
			local recovertime = math.max(0, 275 - 15 * parent.Sharpness)
		...
		elseif self.swing == true then
			...
			if self.connect == true or self.num > math.pi * 1.5 then
				...
				local part = CreateMOPixel("Particle Powersword 1")
				part.Pos = self.MuzzlePos

				part.Vel = parent.Vel + Vector((85 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)

				part.Mass = 0.07 * RangeRand(0.75, 1.25)
				part.Sharpness = 120 * RangeRand(0.75, 1.25)
