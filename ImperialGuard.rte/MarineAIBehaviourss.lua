...
-- function imperialguardLines.createSoundEffect(self, effectName, variations)
...
function imperialguardLines.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						imperialguardLines.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
...
function imperialguardLines.createEmotion(self, emotion, priority, duration, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
...
function imperialguardLines.handleMovement(self)
...
function imperialguardLines.handleHealth(self)
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			self.FGArmLost = true;
			imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.suppressedHigh, 5, 4);
		end
		...
		elseif wasLightlyInjured then
			imperialguardLines.createEmotion(self, 2, 1, 500);
		...
		if (wasInjured) or (wasHeavilyInjured) and self.Head then
			if self.Health > 0 then
				imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.Pain, 2, 2)
			else
				imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.Death, 10, 4)
			end
		end		
	end
...
function imperialguardLines.handleDying(self)
	if self.Head then
...
function imperialguardLines.handleSuppression(self)
	...
	if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		if self.Head.Frame == self.baseHeadFrame then
			imperialguardLines.createEmotion(self, 1, 0, 100);
		...
		if self.Suppression > 25 then
			if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) then
				...
				elseif self.Suppression > 80 then
					imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 4, 4);
					self.suppressedVoicelineTimer:Reset();
					self.suppressionUpdates = 0;
				end
				if self.Suppressed == false then -- initial voiceline
					if self.Suppression > 55 then
						...
						imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.suppressedLow, 3, 2);
					end
					self.suppressedVoicelineTimer:Reset();
				end
			end
...
function imperialguardLines.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
	-- wimperialguards off of the native AI's target
		...
		if self.spotAllowed ~= false then
			...
			if self.LastTargetID == -1 then
				...
				then
					...
					imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3);
...
function imperialguardLines.handleDying(self)
	if self.Head then
		if self.deathSoundPlayed ~= true then
			self.deathSoundPlayed = true;
			imperialguardLines.createVoiceSoundEffect(self, self.voiceSounds.Death, 10);
		end		
	end
end
