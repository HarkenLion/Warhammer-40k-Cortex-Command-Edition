...
function ArtilleryLight(self)
	...
	if self.salvoTimer:IsPastSimMS(1500) and self.artilleryTimer:IsPastSimMS(1500) then
		-- create new shell
		self.Bomb = CreateTDExplosive("Standard Bomb")
		self.Bomb.Pos = Vector(self.Pos.X + math.random(-1500, 1500), -10)
		self.Bomb.Vel = Vector(math.random(6, 10), 150 + math.random(20))
		MovableMan:AddParticle(self.Bomb)
		self.BombsThrown = self.BombsThrown + 1
		self.BombardTimer:Reset()
		self.Bomb2 = CreateTDExplosive("Standard Bomb")
		self.Bomb2.Pos = Vector(self.Pos.X + math.random(-1500, 1500), -10)
