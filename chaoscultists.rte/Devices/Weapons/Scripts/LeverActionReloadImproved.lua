...
function Create(self)
	...
	self.reloadAngleSize = 0.2 --Angle size when reloading.			Change these two depending on weapon
...
function Update(self)
	...
	if self.Magazine ~= nil then
		...
		if self.reloadOpenTimer:IsPastSimMS(50) then --Jesus this took a while to figure out.	Should avoid it spamming the sound when trying to fire mid-reload.
			if self.reloadOpenSound == true then
				local soundEffect = CreateAEmitter("Guard Lever Action Reload Open", "GrandGuard.rte")
	...
	if self.reloadCycle == false then
		if self.reloadCloseSound == true then
			local soundEffect = CreateAEmitter("Guard Lever Action Reload Close", "GrandGuard.rte")
			soundEffect.Pos = self.Pos
			MovableMan:AddParticle(soundEffect)
			self.reloadCloseSound = false
		end
		...
		if self.chamber == true then --Time to pull the lever
			if self.pullTimer:IsPastSimMS(15000 / self.RateOfFire) then
				...
				if self.reloadFullSound == true then
					local soundEffect = CreateAEmitter("Guard Lever Action Reload Full", "GrandGuard.rte")
					soundEffect.Pos = self.Pos
					MovableMan:AddParticle(soundEffect)
