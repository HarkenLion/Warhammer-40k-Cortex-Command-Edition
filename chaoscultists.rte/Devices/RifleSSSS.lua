...
function OnFire(self)
	if self.RootID ~= self.ID then
		...
		local sfx = CreateAEmitter("Lass Muzzle Flash")
		...
		if rayL > -1 then
			...
			local pulseexplosion = CreateMOPixel("Rifle Las Particle Light HitSSS")
			...
			local pulseexplosion = CreateMOPixel("Rifle Las Particle HitSSS")
			pulseexplosion.Pos = hitpos
			pulseexplosion.Team = self.Team
			...
			PrimitiveMan:DrawLinePrimitive(self.MuzzlePos, hitpos, 13, 3)
			...
			PrimitiveMan:DrawLinePrimitive(self.MuzzlePos, self.MuzzlePos + self:RotateOffset(Vector(1900, 0)), 13, 3)
		end
