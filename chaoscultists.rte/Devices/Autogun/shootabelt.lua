...
function Update(self)
	...
	self.cablelength = self.RoundCount * 0.05 --LENGTH OF PRIMITIVE DRAWING CABLE CHANGES based on AMOUNT OF ROUNDS LEFT to ALTER LENGTH OF CHAIN
	--HANDLE CABLE
	local i = 6
	...
	while i > -1 do
		if i <= 5 - self.RoundCount * 0.05 then
			...
			local usepos = self.Pos + Vector(2 * FlipFactor, -4):RadRotate(self.RotAngle)
			self.apx[i] = usepos.X
			self.apy[i] = usepos.Y
		else
