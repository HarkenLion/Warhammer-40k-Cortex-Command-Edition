require("Actors/AI/ArmadaBehaviors")

NativeArmadaAI = {}

function NativeArmadaAI:Create(Owner)
	local Members = {}
	...
	-- set shooting skill
	Members.aimSpeed, Members.aimSkill, Members.skill = ArmadaBehaviors.GetTeamShootingSkill(Owner.Team)
	-- default to enhanced AI if AI skill has been set high enough
	if
		Members.skill >= GameActivity.NUTSDIFFICULTY
		or Owner:HasObjectInGroup("Brains")
		or Owner:HasObjectInGroup("Actors - Snipers")
	then
		Members.SpotTargets = ArmadaBehaviors.CheckEnemyLOS
	else
		Members.SpotTargets = ArmadaBehaviors.LookForTargets
	end
	...
	if Owner.Jetpack then
		...
		Members.jetBurstFactor = (
				Owner.Jetpack:EstimateImpulse(true) * GetPPM() / TimerMan.DeltaTimeSecs - Members.jetImpulseFactor
			)
			* math.pow(TimerMan.DeltaTimeSecs, 2)
			* 0.5
...
function NativeArmadaAI:Update(Owner)
	...
	if self.AirTimer:IsPastSimMS(120) then
		...
		if #Origin == 0 then
			table.insert(
				Origin,
				Vector(Owner.Pos.X, Owner.Pos.Y)
					+ Vector(0, 4 + ToMOSprite(Owner):GetSpriteHeight() + Owner.SpriteOffset.Y)
			)
		end
		for i = 1, #Origin do
	...
	if FoundMO then
		...
		if
			self.Behavior ~= nil
			and self.Target
			and MovableMan:ValidMO(self.Target)
			and FoundMO.ID == self.Target.ID
		then -- found the same target
		...
		elseif FoundMO.Team ~= Owner.Team then -- found an enemy
			...
			elseif
				...
				and FoundMO.Team ~= Activity.NOTEAM
				and Owner.AIMode ~= Actor.AIMODE_SENTRY
				and ToADoor(FoundMO).Door
				and ToADoor(FoundMO).Door:IsAttached()
			then
			...
			if FoundMO and FoundMO.Status < Actor.INACTIVE then
				if self.Target then
					...
					if
						ArmadaBehaviors.CalculateThreatLevel(FoundMO, Owner)
						> ArmadaBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
					then
						self.OldTargetPos = Vector(self.Target.Pos.X, self.Target.Pos.Y)
						self.Target = FoundMO
		...
		if self.ReloadTimer:IsPastSimMS(8000) then -- check if we need to reload
			...
			elseif not ArmadaBehaviors.EquipPreferredWeapon(self, Owner) then -- make sure we equip a preferred or a primary weapon if we have one
				self.ReloadTimer:Reset()
			end
		end
	end
						...
						if dist < radius then
							...
							if Leader.EquippedItem then
								...
								if IsHDFirearm(Leader.EquippedItem) then
									...
									self.Ctrl.AnalogAim = (
										aimDelta * (1 - aimCorrectionRatio)
										+ SceneMan:ShortestDistance(Owner.Pos, Leader.ViewPoint + aimDelta, false)
											* aimCorrectionRatio
									).Normalized
									...
									if LeaderWeapon:IsWeapon() then
										...
										if Owner.FirearmIsReady then
											...
											if
												IsHDFirearm(Owner.EquippedItem)
												and Leader:GetController():IsState(Controller.WEAPON_FIRE)
											then
												...
												if OwnerWeapon:IsTool() then
													...
													if
														...
														and not Owner:EquipDeviceInGroup("Weapons - Primary", true)
													then
														Owner:EquipFirearm(true)
													end
												elseif
													LeaderWeapon:GetAIBlastRadius()
														>= OwnerWeapon:GetAIBlastRadius() * 0.5
													and OwnerWeapon:CompareTrajectories(LeaderWeapon)
														< math.max(100, OwnerWeapon:GetAIBlastRadius())
												then
								...
								elseif
									IsThrownDevice(Leader.EquippedItem)
									and Leader:IsPlayerControlled()
									and ToThrownDevice(Leader.EquippedItem):HasObjectInGroup("Bombs - Grenades")
									and Owner:HasObjectInGroup("Bombs - Grenades")
								then
	...
	if self.squadShoot then
		...
		if
			Owner.FirearmIsSemiAuto
			and self.SquadShootTimer:IsPastSimMS(Owner.FirearmActivationDelay + self.SquadShootDelay)
		then
			...
			if not msg then
				ConsoleMan:PrintString(
					Owner.PresetName
						...
						.. " error:\n"
						.. done
						.. debug.traceback(self.GoToBehavior)
		...
		elseif not (self.Target or self.UnseenTarget) then
			...
			if Owner.Health < (Owner.MaxHealth * 0.5) then
				...
				elseif Owner:HasObject("Health Syringe") then
					self.useMedikit = Owner:EquipNamedDevice("Health Syringe", true)
				else
					self.useMedikit = false
				...
				if self.AlarmTimer:IsPastSimTimeLimit() and ArmadaBehaviors.ProcessAlarmEvent(self, Owner) then
...
function NativeArmadaAI:Destroy(Owner)
	-- Tell the coroutines to abort to avoid memory leaks
	if self.GoToBehavior then
		local msg, done = coroutine.resume(self.GoToBehavior, self, Owner, true)
	end
...
-- functions that create behaviors. the default behaviors are stored in the ArmadaBehaviors table. store your custom behaviors in a table to avoid name conflicts between mods.
function NativeArmadaAI:CreateSentryBehavior(Owner)
	if self.Target then
		self:CreateAttackBehavior(Owner)
	else
		...
		self.NextBehavior = coroutine.create(ArmadaBehaviors.Sentry) -- replace "ArmadaBehaviors.Sentry" with the function name of your own sentry behavior
		self.NextCleanup = nil
		self.NextBehaviorName = "Sentry"
	end
end

function NativeArmadaAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(ArmadaBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeArmadaAI:CreateGoldDigBehavior(Owner)
	...
	self.NextBehavior = coroutine.create(ArmadaBehaviors.GoldDig)
	self.NextCleanup = nil
	self.NextBehaviorName = "GoldDig"
end

function NativeArmadaAI:CreateBrainSearchBehavior(Owner)
	self.NextBehavior = coroutine.create(ArmadaBehaviors.BrainSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "BrainSearch"
end

function NativeArmadaAI:CreateGetToolBehavior(Owner)
	if Owner.AIMode ~= Actor.AIMODE_SQUAD then
		self.NextBehavior = coroutine.create(ArmadaBehaviors.ToolSearch)
		self.NextCleanup = nil
		self.NextBehaviorName = "ToolSearch"
	end
end

function NativeArmadaAI:CreateGetWeaponBehavior(Owner)
	if Owner.AIMode ~= Actor.AIMODE_SQUAD then
		self.NextBehavior = coroutine.create(ArmadaBehaviors.WeaponSearch)
		self.NextCleanup = nil
		self.NextBehaviorName = "WeaponSearch"
	end
end

function NativeArmadaAI:CreateGoToBehavior(Owner)
	self.NextGoTo = coroutine.create(ArmadaBehaviors.GoToWpt)
...
function NativeArmadaAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()
	...
	if IsADoor(self.Target) and Owner.AIMode ~= Actor.AIMODE_SQUAD then
		...
		if Owner:EquipDeviceInGroup("Tools - Breaching", true) then
			self.NextBehavior = coroutine.create(ArmadaBehaviors.AttackTarget)
			self.NextBehaviorName = "AttackTarget"
		elseif
			Owner.FirearmIsReady
			and ArmadaBehaviors.GetProjectileData(Owner).pen * 0.9
				> (self.Target.Door or self.Target).Material.StructuralIntegrity
		then
			self.NextBehavior = coroutine.create(ArmadaBehaviors.ShootTarget)
	...
	elseif
		...
		and Owner.AIMode ~= Actor.AIMODE_SENTRY
		and self.Target.HFlipped == Owner.HFlipped
		and Owner:EquipDeviceInGroup("Bombs - Grenades", true)
		and dist:MagnitudeIsGreaterThan(100)
		and dist:MagnitudeIsLessThan(
			ToThrownDevice(Owner.EquippedItem):GetCalculatedMaxThrowVelIncludingArmThrowStrength() * GetPPM()
		)
		and (self.Target.Pos.Y + 20) > Owner.Pos.Y
	then
		self.NextBehavior = coroutine.create(ArmadaBehaviors.ThrowTarget)
	...
	elseif Owner.AIMode ~= Actor.AIMODE_SQUAD and Owner:EquipDiggingTool(true) and dist:MagnitudeIsLessThan(250) then
		...
		if self.PickUpTimer:IsPastSimMS(2500) then
			...
			if
				not (self.isPlayerOwned and Owner.AIMode == Actor.AIMODE_SENTRY)
				and (self.Target.ClassName == "AHuman" or self.Target.ClassName == "ACrab")
			then
				self.NextBehavior = coroutine.create(ArmadaBehaviors.AttackTarget)
				self.NextBehaviorName = "AttackTarget"
			else
...
function NativeArmadaAI:CreateHtHBehavior(Owner) -- has to be digger, not just "tool"
	if
		Owner.AIMode ~= Actor.AIMODE_SQUAD
		and self.Target
		and (Owner:HasObjectInGroup("Tools - Diggers") or Owner:HasObjectInGroup("Weapons - Melee"))
	then
		self.NextBehavior = coroutine.create(ArmadaBehaviors.AttackTarget)
		self.NextBehaviorName = "AttackTarget"
		self.NextCleanup = function(AI)
...
function NativeArmadaAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(ArmadaBehaviors.ShootArea)
...
function NativeArmadaAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(ArmadaBehaviors.FaceAlarm)
	self.NextBehaviorName = "FaceAlarm"
	self.NextCleanup = nil
end

function NativeArmadaAI:CreatePinBehavior(Owner)
	if self.OldTargetPos and Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(ArmadaBehaviors.PinArea)
