require("Infantry/AI/ArmadaBehaviors")
require("Infantry//AI/ArmadaDroneBehaviors")
require("Infantry//AI/ArmadaTurretBehaviors")

NativeArmadaTurretAI = {}

function NativeArmadaTurretAI:Create(Owner)
	local Members = {}
...
function NativeArmadaTurretAI:Update(Owner)
			...
			if MovableMan:ValidMO(Owner.MOMoveTarget) then
				...
				if not self.Target and self.lateralMoveState == Actor.LAT_STILL then
					...
					if SceneMan:ShortestDistance(Owner.Pos, Leader.Pos, false).Largest < Owner.Height then
						...
						self.Ctrl.AnalogAim =
							SceneMan:ShortestDistance(Owner.EyePos, Leader.ViewPoint, false).Normalized
					end
				end
...
function NativeArmadaTurretAI:Destroy(Owner)
	-- Tell the coroutines to abort to avoid memory leaks
	if self.Behavior then
		local msg, done = coroutine.resume(self.Behavior, self, Owner, true)
	end
...
function NativeArmadaTurretAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(TurretBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeArmadaTurretAI:CreateSentryBehavior(Owner)
...
function NativeArmadaTurretAI:CreateAttackBehavior(Owner)
...
function NativeArmadaTurretAI:CreateSuppressBehavior(Owner)
	if Owner.FirearmIsReady then
...
function NativeArmadaTurretAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(HumanBehaviors.FaceAlarm)
	self.NextBehaviorName = "FaceAlarm"
	self.NextCleanup = nil
end

function NativeArmadaTurretAI:CreatePinBehavior(Owner)
