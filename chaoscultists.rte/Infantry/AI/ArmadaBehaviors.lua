...
function ArmadaBehaviors.GetTeamShootingSkill(team)
	local skill = 50
...
function ArmadaBehaviors.SetShootingSkill()
	-- returns average skill for all active teams
	AI.aimSpeed, AI.aimSkill = ArmadaBehaviors.GetTeamShootingSkill(-1)
end

function ArmadaBehaviors.GetShootingSkill()
	-- returns average skill for all active teams
	return ArmadaBehaviors.GetTeamShootingSkill(-1)
end

-- spot targets by casting a ray in a random direction
function ArmadaBehaviors.LookForTargets(AI, Owner, Skill)
	...
	if FoundMO then
		...
		if
			...
			or not SceneMan:IsUnseen(HitPoint.X, HitPoint.Y, Owner.Team)
...
function ArmadaBehaviors.CheckEnemyLOS(AI, Owner, Skill)
	if not AI.Enemies then -- add all enemy actors on our screen to a table and check LOS to them, one per frame
		...
		for Act in MovableMan.Actors do
			if Act.Team ~= Owner.Team then
				if not AI.isPlayerOwned or not SceneMan:IsUnseen(Act.Pos.X, Act.Pos.Y, Owner.Team) then -- AI-teams ignore the fog
					...
					if
						(math.abs(Dist.X) - Act.Diameter < FrameMan.PlayerScreenWidth * (0.4 + Skill * 0.005))
						and (math.abs(Dist.Y) - Act.Diameter < FrameMan.PlayerScreenHeight * (0.4 + Skill * 0.005))
					then
		...
		return ArmadaBehaviors.LookForTargets(AI, Owner, Skill) -- cast rays like normal actors occasionally
	else
			...
			if MovableMan:ValidMO(Enemy) then
				...
				if Enemy.ClassName == "ADoor" then
					...
					if Door and Door:IsAttached() then
						...
						return ArmadaBehaviors.LookForTargets(AI, Owner, Skill) -- this door is destroyed, cast rays like normal actors
					end
				else
...
function ArmadaBehaviors.CalculateThreatLevel(MO, Owner)
...
function ArmadaBehaviors.ProcessAlarmEvent(AI, Owner)
	AI.AlarmPos = nil

	local loudness, AlarmVec
	local canSupress = not AI.flying
		and Owner.FirearmIsReady
		and Owner.EquippedItem:HasObjectInGroup("Weapons - Explosive")
	for Event in MovableMan.AlarmEvents do
		if Event.Team ~= Owner.Team then -- caused by some other team's activities - alarming!
			...
			if AlarmVec.Largest < loudness then -- only react if the alarm is within hearing range
				...
				if
					not AI.LastAlarmVec
					or SceneMan:ShortestDistance(AI.LastAlarmVec, AlarmVec, false):MagnitudeIsGreaterThan(25)
				then
			...
			elseif
				canSupress
				and Event.Range > 0.5
				and PosRand() > (0.3 / AI.aimSkill)
				and AlarmVec.Largest < FrameMan.PlayerScreenWidth * 1.8
				and (not AI.LastAlarmVec or SceneMan:ShortestDistance(AI.LastAlarmVec, AlarmVec, false).Largest > 30)
			then
				...
				if dot > 0.2 then
					...
					if ID ~= rte.NoMOID then
						...
						if FoundMO then
							...
							if
								not FoundMO.EquippedItem
								or not FoundMO.EquippedItem:HasObjectInGroup("Weapons - Explosive")
							then
							...
							then
								...
								local AimEnemy =
									SceneMan:ShortestDistance(FoundMO.EyePos, FoundMO.ViewPoint, false).Normalized
...
function ArmadaBehaviors.GetGrenadeAngle(AimPoint, TargetVel, StartPos, muzVel)
...
function ArmadaBehaviors.EquipPreferredWeapon(AI, Owner)
...
function ArmadaBehaviors.EquipPrimaryWeapon(AI, Owner)
	...
	while true do
		...
		if SharpTimer:IsPastSimMS(aimTime) then
			...
			if Owner.AIMode ~= Actor.AIMODE_SQUAD then
				if
					SceneMan:ShortestDistance(Owner.Pos, AI.SentryPos, false):MagnitudeIsGreaterThan(Owner.Height * 0.7)
				then
				...
				elseif AI.TargetLostTimer:IsPastSimTimeLimit() and math.random() < Owner.Perceptiveness then
					...
					local backAreaRay = Vector(
						-math.random(FrameMan.PlayerScreenWidth / 4, FrameMan.PlayerScreenWidth / 2) * Owner.FlipFactor,
						0
					):DegRotate(math.random(-25, 25) * Owner.Perceptiveness)
					if
...
function ArmadaBehaviors.Patrol(AI, Owner, Abort)
...
function ArmadaBehaviors.GoldDig(AI, Owner, Abort)
...
function ArmadaBehaviors.BrainSearch(AI, Owner, Abort)
	if AI.PlayerPreferredHD then
		Owner:EquipNamedDevice(AI.PlayerPreferredHD, true)
	end
...
function ArmadaBehaviors.WeaponSearch(AI, Owner, Abort)
	...
	for movableObject in MovableMan:GetMOsInRadius(Owner.Pos, maxSearchDistance, -1, true) do
		...
		if IsHeldDevice(movableObject) and MovableMan:ValidMO(movableObject) then
			...
			if
				device:IsPickupableBy(Owner)
				and not device:IsActivated()
				and device.Vel.Largest < 3
				and not SceneMan:IsUnseen(device.Pos.X, device.Pos.Y, Owner.Team)
			then
				local distanceToDevice = SceneMan:ShortestDistance(
					Owner.Pos,
					device.Pos,
					SceneMan.SceneWrapsX or SceneMan.SceneWrapsY
				)
	...
	table.sort(devices, function(device, otherDevice)
		return device.distance.SqrMagnitude < otherDevice.distance.SqrMagnitude
	end)
		...
		for _, deviceEntry in pairs(devices) do
			...
			if MovableMan:ValidMO(device) then
				...
				if AI.useExpensiveToolAndWeaponSearch then
					pathToItemIsObstructed = SceneMan:CastStrengthRay(
						Owner.Pos,
						deviceEntry.distance,
						5,
						Vector(),
						4,
						rte.grassID,
				...
				local distanceToTarget = pathToItemIsObstructed
						and SceneMan.Scene:CalculatePath(
							Owner.Pos,
							device.Pos,
							false,
							1,
					...
					or (deviceEntry.distance.Magnitude / pathfinderNodeSize)
...
function ArmadaBehaviors.ToolSearch(AI, Owner, Abort)
	local maxSearchDistance
	...
	for movableObject in MovableMan:GetMOsInRadius(Owner.Pos, maxSearchDistance, -1, true) do
		...
		if IsHeldDevice(movableObject) and MovableMan:ValidMO(movableObject) then
			...
			if
				...
				and device.Vel.Largest < 3
				and not SceneMan:IsUnseen(device.Pos.X, device.Pos.Y, Owner.Team)
				and device:HasObjectInGroup("Tools - Diggers")
			then
				local distanceToDevice = SceneMan:ShortestDistance(
					Owner.Pos,
					device.Pos,
...
function ArmadaBehaviors.GetRealVelocity(Owner)
...
function ArmadaBehaviors.UpdateAverageVel(Owner, AverageVel)
	...
	AverageVel = AverageVel + (ArmadaBehaviors.GetRealVelocity(Owner) / ticksPerTime)

	return AverageVel
end

-- move to the next waypoint
function ArmadaBehaviors.GoToWpt(AI, Owner, Abort)
	...
	while true do
		AverageVel = ArmadaBehaviors.UpdateAverageVel(Owner, AverageVel)
		...
		if AI.refuel and Owner.Jetpack then
			...
			if
				Owner.Jetpack.JetTimeLeft > Owner.Jetpack.JetTimeTotal * 0.98
				or (AI.flying and Owner.Vel.Y < -3 and Owner.Jetpack.JetTimeLeft > AI.minBurstTime * 2)
			then
				AI.refuel = false
		...
		elseif WptList then -- we have a list of waypoints, follow it
			if not WptList[1] and not Waypoint then -- arrived
				if Owner.MOMoveTarget then -- following actor
					if Owner.MOMoveTarget:IsActor() then
						...
						if
							Trace.Largest < Owner.Height * 0.5 + (Owner.MOMoveTarget.Height or 100) * 0.5
							and SceneMan:CastStrengthRay(Owner.Pos, Trace, 5, Vector(), 4, rte.grassID, true)
						then -- add a waypoint if the MOMoveTarget is close and in LOS
							Waypoint = {
								Pos = SceneMan:MovePointToGround(Owner.MOMoveTarget.Pos, Owner.Height * 0.2, 4),
							}
						else
							WptList = nil -- update the path
						end
							...
							if Waypoint.Type ~= "air" then
								...
								if Waypoint.Type ~= "drop" and Owner:HasObjectInGroup("Tools - Diggers") then
									...
									then
										if
											SceneMan
												:ShortestDistance(Owner.Pos, Free, false)
												:MagnitudeIsLessThan(Owner.Height * 0.4)
								...
								if digState == AHuman.NOTDIGGING and AI.deviceState ~= AHuman.DIGGING then
									...
									local angleRadians = math.abs(
										math.atan2(-(heading.X * heading.Y), heading.X * heading.X)
									)
									...
									if
										angleDegrees <= crawlThresholdDegrees
										and Owner.Head
				...
				elseif WptList[2] then -- check if some other waypoint is closer
					...
					if RandomWpt then
						...
						if
							mag < 50
							and mag < SceneMan:ShortestDistance(Owner.Pos, Waypoint.Pos, false).Magnitude / 3
						then
								...
								if not AI.flying then
									while true do
										...
										if Owner.MOMoveTarget and MovableMan:ValidMO(Owner.MOMoveTarget) then
											...
											if
												Trace.Largest
													> Owner.Height * 0.4 + (Owner.MOMoveTarget.Height or 100) * 0.4
												or SceneMan:CastStrengthRay(
													Owner.Pos,
													Trace,
													5,
							...
							if not AI.Target and Owner:EquipDiggingTool(true) then -- switch to the digger if we have one
								if Owner.FirearmIsEmpty then -- reload if it's empty
									...
									if AI.teamBlockState == Actor.BLOCKED then
										...
										if
											SceneMan
												:ShortestDistance(PrevWptPos, Owner.Pos, false)
												:MagnitudeIsGreaterThan(Owner.Height * 0.5)
											and SceneMan
												:ShortestDistance(Owner.Pos, Waypoint.Pos, false)
												:MagnitudeIsGreaterThan(Owner.Height * 0.5)
										then
											...
											if SceneMan:CastNotMaterialRay(Owner.Pos, Ray, 0, 3, false) < 0 then
												...
												then
													...
													Ray = Vector(Owner.Height * 0.3, 0):RadRotate(
														centerAngle - sweepRange
													) -- down
													if
														SceneMan:CastNotMaterialRay(Owner.Pos, Ray, rte.airID, 3, false)
														< 0
													then
											...
											if math.abs(angDiff) < 0.1 then
												...
												AI.Ctrl.AnalogAim = (Vector(AimVec.X, AimVec.Y):RadRotate(
													-angDiff * 0.15
												)).Normalized
											end
											...
											if
												SceneMan:ShortestDistance(Owner.Pos, Waypoint.Pos, false).Largest
												< Owner.Height * 0.3
											then
												if
													not SceneMan:CastStrengthRay(
											...
											elseif Owner.AIMode == Actor.AIMODE_GOLDDIG then
												Waypoint.Pos = SceneMan:MovePointToGround(
													Waypoint.Pos,
													Owner.Height * 0.2,
													4
												)
							...
							elseif ArrivedTimer:IsPastSimTimeLimit() then -- only remove a waypoint if we have been close to it for a while
								if Waypoint.Type == "last" then
									if not AI.flying and Owner.Vel.Largest < 5 then
										if not Owner.MOMoveTarget then
											local ProxyWpt = SceneMan:MovePointToGround(
												Owner:GetLastAIWaypoint(),
												Owner.Height * 0.2,
												4
											)
											if
												SceneMan:ShortestDistance(Owner.Pos, ProxyWpt, false).Largest
												< Owner.Height * 0.4
											then
												Owner:ClearAIWaypoints()
												Owner:ClearMovePath()
									...
									if Owner.Jetpack.JetTimeLeft < AI.minBurstTime then
										...
										if AI.Target and AI.canHitTarget and AI.BehaviorName ~= "AttackTarget" then
											...
											if AI.flying then
												...
												if Owner.Jetpack:CanTriggerBurst() then
													t = math.max(
														math.min(
															0.4,
															Owner.Jetpack.JetTimeLeft * 0.001
																- TimerMan.AIDeltaTimeSecs * 10
														),
														TimerMan.AIDeltaTimeSecs
													)
												end

												-- test jumping
												local JetAccel = Accel
													+ Vector(-jetStrength, 0):RadRotate(
														Owner.RotAngle
															+ 1.375 * math.pi
															+ Owner:GetAimAngle(false) * 0.25
													)
												...
												if jumpScore < 0 then -- no obstacles: calculate the distance from the future pos to the wpt
													...
													jumpScore = SceneMan:ShortestDistance(Waypoint.Pos, JumpPos, false).Largest
														* 2
												end
												...
												if
													SceneMan
														:ShortestDistance(Waypoint.Pos, FallPos, false)
														:MagnitudeIsLessThan(jumpScore)
												then
													AI.jump = false
												else
													AI.jump = true
											...
											for k, Face in pairs(Facings) do
												local JetAccel = Vector(-jetStrength, 0):RadRotate(
													Owner.RotAngle + 1.375 * math.pi + Face.facing * 0.25
												)
												local JumpPos = Owner.Head.Pos
													+ PixelVel
													+ (Accel + JetAccel) * (t * t * 0.5)
												...
												-- check for obstacles from the head
												Trace = SceneMan:ShortestDistance(Owner.Head.Pos, JumpPos, false)
												local obstDist = SceneMan:CastObstacleRay(
													Owner.Head.Pos,
													Trace,
													JumpPos,
													Vector(),
												...
												if obstDist < 0 then -- no obstacles: calculate the distance from the future pos to the wpt
													Facings[k].range = SceneMan:ShortestDistance(
														Waypoint.Pos,
														JumpPos,
														false
													).Magnitude
											...
											local delta = SceneMan:ShortestDistance(Waypoint.Pos, FallPos, false).Magnitude
												- Facings[1].range
											if delta < 1 then
												AI.jump = false
			...
			if
				...
				and not SceneMan:CastStrengthRay(Owner.Pos, Trace, 105, Vector(), 1, rte.grassID, true)
			then
				...
				if not Owner.MOMoveTarget then
					...
					if
						Trace.Largest < Owner.Height * 0.25
						and not SceneMan:CastStrengthRay(Owner.Pos, Trace, 6, Vector(), 3, rte.grassID, true)
					then
						if Owner.AIMode == Actor.AIMODE_GOTO then
							AI.SentryPos = Vector(Owner.Pos.X, Owner.Pos.Y)
							AI:CreateSentryBehavior(Owner)
				...
				for _, WptPos in pairs(PathDump) do
					...
					if
						math.abs(Dist.Y) > 30
						or Dist:MagnitudeIsGreaterThan(80) -- skip any waypoint too close to the previous one
							...
							for _, LZ in pairs(GapList) do
								...
								then
									...
									then -- TODO: check LOS? what if 0?
										table.insert(
											WptList,
											{ Pos = Free + Vector(0, Owner.Height / 3), Type = "air" }
										) -- guide point in the air
										NextWpt = LZ.Wpt
		...
		-- movement commands
		if
			(AI.Target and AI.BehaviorName ~= "AttackTarget")
			or (
				Owner.AIMode ~= Actor.AIMODE_SQUAD
				and (AI.BehaviorName == "ShootArea" or AI.BehaviorName == "FaceAlarm")
			)
		then
...
function ArmadaBehaviors.GoProne(AI, Owner, TargetPos, targetID)
...
function ArmadaBehaviors.GetProjectileData(Owner)
...
function ArmadaBehaviors.ShootTarget(AI, Owner, Abort)
	...
	if not AI.flying and AI.Target.Vel.Largest < 4 and ArmadaBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID) then
	...
	while true do
		...
		if Owner.FirearmIsReady then
			...
			if not PrjDat or PrjDat.MagazineName ~= Weapon.Magazine.PresetName then
				PrjDat = ArmadaBehaviors.GetProjectileData(Owner)
				...
				if
					(
						(Weapon.Magazine.Capacity > -1 and Weapon.Magazine.Capacity < 10)
						or Weapon:HasObjectInGroup("Weapons - Sniper")
					) and Dist.Largest > 100
				then
				...
				if checkAim then
					...
					if range < PrjDat.blast then
						...
						if Owner.InventorySize > 0 then -- we have more things in the inventory
							if
								range < 60
								and (
									Owner:HasObjectInGroup("Tools - Diggers")
									or Owner:HasObjectInGroup("Weapons - Melee")
								)
							then
					...
					elseif range < PrjDat.rng then
						...
						if (timeToTarget * TargetAvgVel.Magnitude) > 2 then
							...
							Dist = SceneMan:ShortestDistance(
								Weapon.Pos,
								AimPoint + (Owner.Vel * 0.5 + TargetAvgVel) * timeToTarget,
								false
							)
						end

						aimTarget = ArmadaBehaviors.GetAngleToHit(PrjDat, Dist)
						if aimTarget then
							AI.canHitTarget = true
						else
							AI.canHitTarget = false
							...
							if not AI.isPlayerOwned or Owner.AIMode ~= Actor.AIMODE_SENTRY then
								...
								then -- move towards the target
									local OldWaypoint = SceneMan:MovePointToGround(
										Owner:GetLastAIWaypoint(),
										Owner.Height / 5,
										4
					...
					elseif
						not AI.isPlayerOwned
						or not (Owner.AIMode == Actor.AIMODE_SENTRY or Owner.AIMode == Actor.AIMODE_SQUAD)
					then -- target out of reach; move towards it
					...
					if LOSTimer:IsPastSimTimeLimit() then
						...
						if
							(range < Owner.AimDistance + Weapon.SharpLength + FrameMan.PlayerScreenWidth * 0.5)
							and (
				...
				local angChange = math.max(
					math.min(angDiff * (0.1 / AI.aimSkill), 0.17 / AI.aimSkill),
					-0.17 / AI.aimSkill
				)
					...
					if AI.canHitTarget and angDiff < 0.7 then
						...
						if openFire > 0 and PrjDat.blast > 0 then
							if
								SceneMan:CastObstacleRay(
									Weapon.MuzzlePos,
									Weapon:RotateOffset(Vector(50, 0)),
									Vector(),
									Vector(),
									Owner.ID,
									Owner.IgnoresWhichTeam,
				...
				if Owner.FirearmIsEmpty then
					...
					if AI.Target then
						...
						if Owner:EquipLoadedFirearmInGroup("Weapons - Primary", "None", true) then
							...
							if
								Dist.Largest < (FrameMan.PlayerScreenWidth * 0.5 + AI.Target.Radius + Owner.AimDistance)
							then
	...
	if
		...
		and (Owner.EquippedItem and Owner.EquippedItem:HasObjectInGroup("Weapons - Secondary"))
	then
...
function ArmadaBehaviors.ThrowTarget(AI, Owner, Abort)
				...
				if ID > 0 and ID ~= rte.NoMOID then -- MO found
						...
						if MovableMan:ValidMO(MO) then
							if MO.Team ~= Owner.Team then
								...
								elseif MO.ClassName == "ACrab" then
									...
									local Legs = AI.Target.LeftFGLeg
										or AI.Target.RightFGLeg
										or AI.Target.LeftBGLeg
										or AI.Target.RightFGLeg -- the legs
									if Legs then
										AimPoint = Legs.Pos
									end
					...
					if Owner.ThrowableIsReady then
						...
						if Grenade then
							...
							aim = ArmadaBehaviors.GetGrenadeAngle(AimPoint, Vector(), Grenade.MuzzlePos, maxThrowVel)
							if aim then
								...
								aim = ArmadaBehaviors.GetGrenadeAngle(
									AimPoint,
									Vector(),
									Grenade.MuzzlePos,
									(maxThrowVel + minThrowVel) * 0.5
								)
								if aim then
...
function ArmadaBehaviors.AttackTarget(AI, Owner, Abort)
	...
	while true do
		...
		if AI.Target.ClassName == "ADoor" then
			...
			suitableWeapon = Owner:EquipDeviceInGroup("Tools - Breaching", true)
				or Owner:EquipDeviceInGroup("Tools - Diggers", true)
				or Owner:EquipDeviceInGroup("Weapons - Melee", true)
		else
			-- Prefer melee weapons for attacking actors
			suitableWeapon = Owner:EquipDeviceInGroup("Weapons - Melee", true)
				or Owner:EquipDeviceInGroup("Tools - Diggers", true)
				or Owner:EquipDeviceInGroup("Tools - Breaching", true)
		end
		...
		local attackPos = (
					AI.Target.ClassName == "ADoor"
					and ToADoor(AI.Target).Door
					and ToADoor(AI.Target).Door:IsAttached()
				)
				and ToADoor(AI.Target).Door.Pos
			or AI.Target.Pos
...
function ArmadaBehaviors.GetAngleToHit(PrjDat, Dist)
...
function ArmadaBehaviors.ShootArea(AI, Owner, Abort)
	...
	if
		...
		and ArmadaBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID)
	then
		aimTime = aimTime + 500
	end
	...
	local aim
	local PrjDat = ArmadaBehaviors.GetProjectileData(Owner)
	...
	if Dist:MagnitudeIsLessThan(PrjDat.rng) then
		aim = ArmadaBehaviors.GetAngleToHit(PrjDat, Dist)
	else
	...
	while aim do
		...
		if AI.UnseenTarget and CheckTargetTimer:IsPastRealMS(400 * AI.aimSkill) then
			if
				MovableMan:ValidMO(AI.UnseenTarget)
				and (AI.UnseenTarget.ClassName == "AHuman" or AI.UnseenTarget.ClassName == "ACrab")
			then
				CheckTargetTimer:Reset()
				if
					AI.UnseenTarget:GetController() and AI.UnseenTarget:GetController():IsState(Controller.WEAPON_FIRE)
				then
					-- compare the enemy aim angle with the angle to us
					local AimEnemy =
						SceneMan:ShortestDistance(
							AI.UnseenTarget.EyePos,
							AI.UnseenTarget.ViewPoint,
							false
						).Normalized
...
function ArmadaBehaviors.FaceAlarm(AI, Owner, Abort)
	if AI.AlarmPos then
...
function ArmadaBehaviors.PinArea(AI, Owner, Abort)
...
local Proxy = {}
local Mt = {
	__index = ArmadaBehaviors,
	__newindex = function(Table, k, v)
		error("The ArmadaBehaviors table is read-only.", 2)
	end,
}
setmetatable(Proxy, Mt)
ArmadaBehaviors = Proxy
