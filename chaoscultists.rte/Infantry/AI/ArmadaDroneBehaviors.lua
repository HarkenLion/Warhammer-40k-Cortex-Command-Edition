ArmadaDroneBehaviors = {}

function ArmadaDroneBehaviors.LookForTargets(AI, Owner)
	...
	if FoundMO then
		...
		if
			AI.isPlayerOwned
			and SceneMan:IsUnseen(HitPoint.X, HitPoint.Y, Owner.Team)
			...
			if FoundMO.Team == Owner.Team then -- found an ally
				if AI.Target then
					if
						...
						< SceneMan:ShortestDistance(Owner.Pos, AI.Target.Pos, false).Magnitude
					then
				...
				elseif
					FoundMO.ClassName == "ADoor"
					and HumanBehaviors.GetProjectileData(Owner).pen * 0.9
						> ToADoor(FoundMO).Door.Material.StructuralIntegrity
				then
				...
				if FoundMO then
					if AI.Target then
						...
						if
							HumanBehaviors.CalculateThreatLevel(FoundMO, Owner)
							> HumanBehaviors.CalculateThreatLevel(AI.Target, Owner) + 0.2
						then
...
function ArmadaDroneBehaviors.Sentry(AI, Owner, Abort)
	local sweepUp = true
	...
	if Owner.HFlipped ~= AI.SentryFacing then
		Owner.HFlipped = AI.SentryFacing -- turn to the direction we have been order to Armada
	...
	while true do -- scan the area for obstacles
		...
		if
			not SceneMan:CastStrengthRay(
				Owner.EyePos,
				Vector(60, 0):RadRotate(Owner:GetAimAngle(true)),
				5,
				Hit,
				2,
				0,
...
function ArmadaDroneBehaviors.GoToWpt(AI, Owner, Abort)
	if not Owner.MOMoveTarget then
		if SceneMan:ShortestDistance(Owner:GetLastAIWaypoint(), Owner.Pos, false).Largest < Owner.Height * 0.15 then
			...
			if Owner.AIMode == Actor.AIMODE_GOTO then
				AI.SentryFacing = Owner.HFlipped -- Armada this direction
				AI.SentryPos = Vector(Owner.Pos.X, Owner.Pos.Y) -- Armada this point
	...
	while true do
			...
			if not WptList[1] and not Waypoint then -- arrived
				if Owner.MOMoveTarget then -- following actor
					if MovableMan:ValidMO(Owner.MOMoveTarget) then
						...
						if
							Trace.Largest < Owner.Height * 0.5 + Owner.MOMoveTarget.Radius
					...
					if
						SceneMan:ShortestDistance(Owner:GetLastAIWaypoint(), Owner.Pos, false).Largest
						< Owner.Height * 0.4
					then
						if Owner.AIMode == Actor.AIMODE_GOTO then
							AI.SentryFacing = Owner.HFlipped -- Armada this direction
				...
				if not Waypoint then -- get the next waypoint in the list
					...
					elseif
						not Owner.MOMoveTarget and SceneMan:ShortestDistance(Owner.Pos, Waypoint.Pos, false).X < 10
					then -- the last waypoint
						Owner:ClearMovePath()
						WptList = nil
						Waypoint = nil
					end
		...
		if AI.BlockingMO then
			if
				not MovableMan:ValidMO(AI.BlockingMO)
				or SceneMan:ShortestDistance(Owner.Pos, AI.BlockingMO.Pos, false).Magnitude
					> (Owner.Diameter + AI.BlockingMO.Diameter) * 1.2
			then
				AI.BlockingMO = nil
			...
			elseif AI.teamBlockState == Actor.NOTBLOCKED and Waypoint then
				if
					...
					or (Waypoint.Pos.X < Owner.Pos.X and AI.BlockingMO.Pos.X < Owner.Pos.X)
				then
...
function ArmadaDroneBehaviors.ShootTarget(AI, Owner, Abort)
		...
		if LOSTimer:IsPastSimTimeLimit() then
			...
			if AimPoint and (not AI.isPlayerOwned or not SceneMan:IsUnseen(AimPoint.X, AimPoint.Y, Owner.Team)) then
				...
				local viewLen = SceneMan:ShortestDistance(Owner.EyePos, Owner.ViewPoint, false).Magnitude
					+ FrameMan.PlayerScreenWidth * 0.55 -- TODO: get AimDistance and SharpLength from the ini
				...
				if Dist.Magnitude < viewLen then
					...
					if
						ID ~= rte.NoMOID and (ID == AI.Target.ID or (MovableMan:GetMOFromID(ID)).RootID == AI.Target.ID)
					then
...
function ArmadaDroneBehaviors.ShootArea(AI, Owner, Abort)
...
local Proxy = {}
local Mt = {
	__index = ArmadaDroneBehaviors,
	__newindex = function(Table, k, v)
		error("The ArmadaDroneBehaviors table is read-only.", 2)
	end,
}
setmetatable(Proxy, Mt)
ArmadaDroneBehaviors = Proxy
