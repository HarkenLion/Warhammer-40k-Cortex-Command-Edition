...
function Create(self)
	...
	self.musclemult = 1 -- --1.25 --2.5 --1.5
...
function Update(self)
	...
	if self.c:IsState(3) then
		...
		if self.FlipFactor == 1 then
			--FORWARD LEG PULL
			if self.FGLeg and self.FGFoot.Pos.X > self.Pos.X and self.FGLeg.RotAngle < self.triggerang then
				local groundray = SceneMan:CastStrengthRay(
					self.FGFoot.Pos,
					Vector(3 * self.FlipFactor, 8):RadRotate(self.FGFoot.RotAngle),
					0,
					terrcheck,
					1,
					0,
			...
			if self.BGLeg and self.BGFoot.Pos.X > self.Pos.X and self.BGLeg.RotAngle < self.triggerang then
				local groundray = SceneMan:CastStrengthRay(
					self.BGFoot.Pos,
					Vector(3 * self.FlipFactor, 8):RadRotate(self.BGFoot.RotAngle),
					0,
					terrcheck,
					1,
					0,
			...
			--BACK LEG PUSH
			if self.FGLeg and self.FGFoot.Pos.X < self.Pos.X - 1 then
				local groundray = SceneMan:CastStrengthRay(
					self.FGFoot.Pos,
					Vector(3 * self.FlipFactor, 8):RadRotate(self.FGFoot.RotAngle),
					0,
				...
				if groundray == true then
					...
					local addforce = self.pscale * xchange * usemass
					--local addforcey = -1*self.pscale*usemass*self.FlipFactor
					local usepos = self.FGLeg.Pos --+Vector(2*self.FlipFactor,8):RadRotate(self.FGLeg.RotAngle)
			...
			if self.BGLeg and self.BGFoot.Pos.X < self.Pos.X - 1 then
				local groundray = SceneMan:CastStrengthRay(
					self.BGFoot.Pos,
					Vector(3 * self.FlipFactor, 8):RadRotate(self.BGFoot.RotAngle),
					0,
				...
				if groundray == true then
					...
					local addforce = self.pscale * xchange * usemass
					--local addforcey = -1*self.pscale*usemass*self.FlipFactor
					local usepos = self.BGLeg.Pos --+Vector(2*self.FlipFactor,8):RadRotate(self.BGLeg.RotAngle)
					local getang = SceneMan:ShortestDistance(usepos, self.Pos, true).AbsRadAngle
					self:AddAbsForce(Vector(addforce, 0):RadRotate(getang), self.Pos)
					--PrimitiveMan:DrawCircleFillPrimitive(usepos, 2, 5);
				end
