...
function NativeNobAI:Create(Owner)
	local Members = {}
...
function NativeNobAI:Update(Owner)
	...
	if self.isBrain then
		FoundMO, HitPoint = OrkBehaviors.CheckEnemyLOS(self, Owner)
	else
		FoundMO, HitPoint = OrkBehaviors.LookForTargets(self, Owner)
	end
			...
			if FoundMO.Team == Owner.Team then -- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if
							OrkBehaviors.CalculateThreatLevel(FoundMO, Owner)
							> OrkBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
						then
	...
	elseif self.flying then -- avoid falling damage
		if (not self.jump and Owner.Vel.Y > 13) or (self.jump and Owner.Vel.Y > 6) then
			self.jump = true
	...
	if not self.Target and not self.UnseenTarget then
		if self.AlarmTimer:IsPastSimTimeLimit() and OrkBehaviors.ProcessAlarmEvent(self, Owner) then
...
-- functions that create behaviors. the default behaviors are stored in the OrkBehaviors table. store your custom behaviors in a table to avoid name conflicts between mods.
function NativeNobAI:CreateSentryBehavior(Owner)
	...
	self.NextBehavior = coroutine.create(OrkBehaviors.Sentry) -- replace "OrkBehaviors.Sentry" with the function name of your own sentry behavior
	self.NextCleanup = nil
	self.NextBehaviorName = "Sentry"
end

function NativeNobAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(OrkBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeNobAI:CreateGoldDigBehavior(Owner)
	self.NextBehavior = coroutine.create(OrkBehaviors.GoldDig)
	self.NextCleanup = nil
	self.NextBehaviorName = "GoldDig"
end

function NativeNobAI:CreateBrainSearchBehavior(Owner)
	self.NextBehavior = coroutine.create(OrkBehaviors.BrainSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "BrainSearch"
end

function NativeNobAI:CreateGetToolBehavior(Owner)
	self.NextBehavior = coroutine.create(OrkBehaviors.ToolSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "ToolSearch"
end

function NativeNobAI:CreateGetWeaponBehavior(Owner)
	self.NextBehavior = coroutine.create(OrkBehaviors.WeaponSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "WeaponSearch"
end

function NativeNobAI:CreateGoToBehavior(Owner)
	self.NextGoTo = coroutine.create(OrkBehaviors.GoToWpt)
...
function NativeNobAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()

	if
		Owner:EquipDiggingTool(true)
		and SceneMan:ShortestDistance(Owner.Pos, self.Target.Pos, false).Magnitude < 305
		and (self.Target.Pos.Y - Owner.Pos.Y) <= 90
	then
		self.NextBehavior = coroutine.create(OrkBehaviors.AttackTarget)
		self.NextBehaviorName = "AttackTarget"
	elseif
		Owner:EquipFirearm(true)
		and SceneMan:ShortestDistance(Owner.Pos, self.Target.Pos, false).Magnitude < 575
		and (self.Target.Pos.Y - Owner.Pos.Y) > 90
	then
		self.NextBehavior = coroutine.create(OrkBehaviors.ShootTarget)
	...
	then
		...
		self.NextBehaviorName = "AttackTarget"
	elseif
		Owner:EquipFirearm(true) and SceneMan:ShortestDistance(Owner.Pos, self.Target.Pos, false).Magnitude > 205
	then
		self.NextBehavior = coroutine.create(OrkBehaviors.ShootTarget)
		self.NextBehaviorName = "ShootTarget"
	elseif Owner:EquipThrowable(true) then
		self.NextBehavior = coroutine.create(OrkBehaviors.ThrowTarget)
...
function NativeNobAI:CreateHtHBehavior(Owner)
...
function NativeNobAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(OrkBehaviors.ShootArea)
...
function NativeNobAI:CreateMoveAroundBehavior(Owner)
	self.NextGoTo = coroutine.create(OrkBehaviors.MoveAroundActor)
...
function NativeNobAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(OrkBehaviors.FaceAlarm)
