...
function OrkBehaviors.LookForTargets(AI, Owner)
	local Origin
	local viewAng
...
function OrkBehaviors.CheckEnemyLOS(AI, Owner)
	if not AI.Enemies then -- add all enemy actors on our screen to a table and check LOS to them, one per frame
		...
		return OrkBehaviors.LookForTargets(AI, Owner) -- cast rays like normal actors occasionally
	else
			...
			if MovableMan:ValidMO(Enemy) then
				...
				if Enemy.ClassName == "ADoor" then
					...
					if Door and Door:IsAttached() then
						...
						return OrkBehaviors.LookForTargets(AI, Owner) -- this door is destroyed, cast rays like normal actors
					end
				else
...
function OrkBehaviors.CalculateThreatLevel(MO, Owner)
...
function OrkBehaviors.ProcessAlarmEvent(AI, Owner)
	AI.AlarmPos = nil
...
function OrkBehaviors.GetGrenadeAngle(AimPoint, TargetVel, StartPos, muzVel)
...
function OrkBehaviors.EquipPrimaryWeapon(AI, Owner)
	if Owner.EquippedItem then
		if
...
function OrkBehaviors.Sentry(AI, Owner)
	local sweepUp = true
	...
	if OrkBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
				if not OrkBehaviors.EquipPrimaryWeapon(AI, Owner) then
...
function OrkBehaviors.Patrol(AI, Owner)
...
function OrkBehaviors.GoldDig(AI, Owner)
...
function OrkBehaviors.BrainSearch(AI, Owner)
	if OrkBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
...
function OrkBehaviors.WeaponSearch(AI, Owner)
...
function OrkBehaviors.ToolSearch(AI, Owner)
	local range, minDist, HD
	local Devices = {}
...
function OrkBehaviors.GoToWpt(AI, Owner)
	...
	while true do
		...
		elseif StuckTimer:IsPastSimTimeLimit() then -- dislodge
			if AI.jump then
				if Owner.Jetpack and Owner.Jetpack.JetTimeLeft < TimerMan.DeltaTimeMS * 3 then -- out of fuel
					AI.jump = false
					AI.refuel = true
					nextLatMove = Actor.LAT_STILL
				else
				...
				if AI.refuel and Owner.Jetpack and Owner.Jetpack.JetTimeLeft > Owner.Jetpack.JetTimeTotal * 0.95 then
					AI.jump = true
				...
				if Waypoint then
					...
					if digState ~= AHuman.NOTDIGGING then
						...
						elseif AI.refuel and Owner.Vel.Y < 5 then
							if
								Owner.Jetpack and Owner.Jetpack.JetTimeLeft > Owner.Jetpack.JetTimeTotal * 0.95
								or (
									AI.flying
									and Owner.Vel.Y < -3
									and Owner.Jetpack
									and Owner.Jetpack.JetTimeLeft > 0.5
								)
							then
								AI.refuel = false
							else
...
function OrkBehaviors.GoProne(AI, Owner, TargetPos, targetID)
...
function OrkBehaviors.GetProjectileData(Owner)
...
function OrkBehaviors.ShootTarget(AI, Owner)
	if not MovableMan:IsActor(AI.Target) then
		Owner.NextBehavior = coroutine.create(OrkBehaviors.AttackTarget)
		return true
	end
	...
	if not AI.flying and AI.Target.Vel.Largest < 4 and OrkBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID) then
	...
	while true do
		...
		if Owner.FirearmIsReady then
			...
			if not PrjDat or PrjDat.MagazineName ~= Weapon.Magazine.PresetName then
				PrjDat = OrkBehaviors.GetProjectileData(Owner)
				...
				if checkAim then
					...
					elseif range < PrjDat.rng then
						...
						aimTarget = OrkBehaviors.GetAngleToHit(PrjDat, Dist)
						if aimTarget then
							AI.canHitTarget = true
						else
							AI.canHitTarget = false
...
function OrkBehaviors.ThrowTarget(AI, Owner)
		...
		if scan < 1 then
			...
			if LOS then -- don't sharp aim until LOS has been confirmed
				if Owner.ThrowableIsReady then
					aim = OrkBehaviors.GetGrenadeAngle(
						AimPoint,
...
function OrkBehaviors.AttackTarget(AI, Owner)
...
function OrkBehaviors.MoveAroundActor(AI, Owner)
	...
	while true do
		...
		if refuel then
			AI.jump = false
			if Owner.Jetpack and Owner.Jetpack.JetTimeLeft > Owner.Jetpack.JetTimeTotal * 0.9 then
				refuel = false
			end
		elseif Owner.Jetpack and Owner.Jetpack.JetTimeLeft < Owner.Jetpack.JetTimeTotal * 0.1 then
			refuel = true
		end
...
function OrkBehaviors.GetAngleToHit(PrjDat, Dist)
...
function OrkBehaviors.ShootArea(AI, Owner)
	...
	if
		...
		and OrkBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID)
	then
		aimTime = aimTime + 500
	end
...
function OrkBehaviors.FaceAlarm(AI, Owner)
...
local Proxy = {}
local Mt = {
	__index = OrkBehaviors,
	__newindex = function(Table, k, v)
		error("The OrkBehaviors table is read-only.", 2)
	end,
}
