...
-- function osog.createSoundEffect(self, effectName, variations)
...
function osog.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						osog.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
...
function osog.createEmotion(self, emotion, priority, duration, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
...
function osog.handleMovement(self)
...
function osog.handleHealth(self)
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			self.FGArmLost = true;
			osog.createVoiceSoundEffect(self, self.voiceSounds.suppressedHigh, 5, 4);
		end
		...
		elseif wasLightlyInjured then
			osog.createEmotion(self, 2, 1, 500);
		...
		if (wasInjured) or (wasHeavilyInjured) and self.Head then
			if self.Health > 0 then
				osog.createVoiceSoundEffect(self, self.voiceSounds.Pain, 2, 2)
			else
				osog.createVoiceSoundEffect(self, self.voiceSounds.Death, 10, 4)
			end
		end		
	end
...
function osog.handleDying(self)
	if self.Head then
...
function osog.handleSuppression(self)
	...
	if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		if self.Head.Frame == self.baseHeadFrame then
			osog.createEmotion(self, 1, 0, 100);
		...
		if self.Suppression > 25 then
			if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) then
				...
				elseif self.Suppression > 80 then
					osog.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 4, 4);
					self.suppressedVoicelineTimer:Reset();
					self.suppressionUpdates = 0;
				end
				if self.Suppressed == false then -- initial voiceline
					if self.Suppression > 55 then
						...
						osog.createVoiceSoundEffect(self, self.voiceSounds.suppressedLow, 3, 2);
					end
					self.suppressedVoicelineTimer:Reset();
				end
			end
...
function osog.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
		...
		if self.spotAllowed ~= false then
			...
			if self.LastTargetID == -1 then
				...
				then
					...
					osog.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3);
...
function osog.handleDying(self)
	if self.Head then
		if self.deathSoundPlayed ~= true then
			self.deathSoundPlayed = true;
			osog.createVoiceSoundEffect(self, self.voiceSounds.Death, 10);
		end		
	end
end
