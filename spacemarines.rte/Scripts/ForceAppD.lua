...
function Create(self)
	...
	local TurretOffsets = {
        ...
        {Position = Vector(-1,-29),Weapon = "Broadsid Shoulder Plasma Pod"}, -- Turret 2
		{Position = Vector(-1,-29),Weapon = "Broadsid Shoulder Plasma Pod"}, -- Turret 3
    }
...
function Update(self)
		...
		if self.FlipFactor == 1 then
			...
			if self.FGLeg and self.FGFoot and self.FGLeg.RotAngle < self.triggerang then
				if self.fglegpushed < 5 then
					local groundray = SceneMan:CastStrengthRay(self.FGFoot.Pos,Vector(3*self.FlipFactor,self.footheight):RadRotate(self.FGFoot.RotAngle),0,terrcheck,1,0,true);
					if groundray == true then
						...
						self:AddAbsForce(Vector(addforce,addforcey):RadRotate(getang), self.Pos+Vector(2*self.FlipFactor,-10))
			...
			if self.BGLeg and self.BGFoot and self.BGLeg.RotAngle < self.triggerang then
				if self.bglegpushed < 5 then
					local groundray = SceneMan:CastStrengthRay(self.BGFoot.Pos,Vector(3*self.FlipFactor,self.footheight):RadRotate(self.BGFoot.RotAngle),0,terrcheck,1,0,true);
					if groundray == true then
	...
	--MOVING LEFT
		if self.FlipFactor == - 1 then
			--FRONT LEG PULL
			if self.FGLeg and self.FGFoot  and self.FGFoot.Pos.X < self.Pos.X and self.FGLeg.RotAngle > math.pi+self.triggerang then
				if self.fglegpushed < 5 then
					local groundray = SceneMan:CastStrengthRay(self.FGFoot.Pos,Vector(3*self.FlipFactor,self.footheight):RadRotate(self.FGFoot.RotAngle),0,terrcheck,1,0,true);
					if groundray == true then
						local xchange = SceneMan:ShortestDistance(Vector(self.FGFootPrevPos,0),Vector(self.FGFoot.Pos.X-self.Pos.X,0), true).Magnitude
						local usemass = (self.FGLeg.Mass+self.FGFoot.Mass)
						local addforce = self.pscale*xchange*usemass
						local addforcey = -1*self.pscale*usemass*self.FlipFactor
						local usepos = self.FGLeg.Pos+Vector(2*self.FlipFactor,-self.kneedist*2)
			...
			if self.BGLeg and self.BGFoot and self.BGFoot.Pos.X < self.Pos.X and self.BGLeg.RotAngle > math.pi+self.triggerang then
				if self.bglegpushed < 5 then
					local groundray = SceneMan:CastStrengthRay(self.BGFoot.Pos,Vector(3*self.FlipFactor,self.footheight):RadRotate(self.BGFoot.RotAngle),0,terrcheck,1,0,true);
					if groundray == true then
						local xchange = SceneMan:ShortestDistance(Vector(self.BGFootPrevPos,0),Vector(self.BGFoot.Pos.X-self.Pos.X,0), true).Magnitude
						local usemass = (self.BGLeg.Mass+self.BGFoot.Mass)
						local addforce = self.pscale*xchange*usemass
						local addforcey = -1*self.pscale*usemass*self.FlipFactor
						local usepos = self.BGLeg.Pos+Vector(2*self.FlipFactor,-self.kneedist*2)
			...
			--BACK LEG PUSH
			if self.FGLeg and self.FGFoot and self.FGFoot.Pos.X > self.Pos.X+1 and self.FGLeg.RotAngle < math.pi+self.triggerangBG then
				if self.fglegpushed > 0 then
					local groundray = SceneMan:CastStrengthRay(self.FGFoot.Pos,Vector(3*self.FlipFactor,self.footheight):RadRotate(self.FGFoot.RotAngle),0,terrcheck,1,0,true);
					if groundray == true then
						local xchange = SceneMan:ShortestDistance(Vector(self.FGFootPrevPos,0),Vector(self.FGFoot.Pos.X-self.Pos.X,0), true).Magnitude
			...
			if self.BGLeg and self.BGFoot and self.BGFoot.Pos.X > self.Pos.X+1 and self.BGLeg.RotAngle < math.pi+self.triggerangBG then
				if self.bglegpushed > 0 then
					local groundray = SceneMan:CastStrengthRay(self.BGFoot.Pos,Vector(3*self.FlipFactor,self.footheight):RadRotate(self.BGFoot.RotAngle),0,terrcheck,1,0,true);
					if groundray == true then
						local xchange = SceneMan:ShortestDistance(Vector(self.BGFootPrevPos,0),Vector(self.BGFoot.Pos.X-self.Pos.X,0), true).Magnitude
