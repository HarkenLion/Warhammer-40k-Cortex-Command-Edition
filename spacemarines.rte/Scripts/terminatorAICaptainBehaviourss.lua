...
-- function taibehaviours.createSoundEffect(self, effectName, variations)
...
function taibehaviours.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						taibehaviours.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
...
function taibehaviours.createEmotion(self, emotion, priority, duration, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
...
function taibehaviours.handleMovement(self)
...
function taibehaviours.handleHealth(self)
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			self.FGArmLost = true;
			taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedHigh, 5, 4);
		end
		...
		elseif wasLightlyInjured then
			taibehaviours.createEmotion(self, 2, 1, 500);
		...
		if (wasInjured) or (wasHeavilyInjured) and self.Head then
			if self.Health > 0 then
				taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.Pain, 2, 2)
			else
				taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.Death, 10, 4)
			end
		end		
	end
...
function taibehaviours.handleDying(self)
	if self.Head then
...
function taibehaviours.handleSuppression(self)
	...
	if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		if self.Head.Frame == self.baseHeadFrame then
			taibehaviours.createEmotion(self, 1, 0, 100);
		...
		if self.Suppression > 25 then
			if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) then
				...
				elseif self.Suppression > 80 then
					taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 4, 4);
					self.suppressedVoicelineTimer:Reset();
					self.suppressionUpdates = 0;
				end
				if self.Suppressed == false then -- initial voiceline
					if self.Suppression > 55 then
						...
						taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedLow, 3, 2);
					end
					self.suppressedVoicelineTimer:Reset();
				end
			end
...
function taibehaviours.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
		...
		if self.spotAllowed ~= false then
			...
			if self.LastTargetID == -1 then
				...
				then
					...
					taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3);
...
function taibehaviours.handleDying(self)
	if self.Head then
		if self.deathSoundPlayed ~= true then
			self.deathSoundPlayed = true;
			taibehaviours.createVoiceSoundEffect(self, self.voiceSounds.Death, 10);
		end		
	end
end