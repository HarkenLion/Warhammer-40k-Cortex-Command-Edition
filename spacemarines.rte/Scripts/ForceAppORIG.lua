...
function Update(self)
	...
	--MOVING RIGHT
		if self.FlipFactor == 1 then
			--FORWARD LEG PULL
			if MovableMan:ValidMO(self.FGLeg) and self.FGFoot.Pos.X > self.Pos.X and self.FGLeg.RotAngle < self.triggerang then
				local groundray = SceneMan:CastStrengthRay(self.FGFoot.Pos,Vector(3*self.FlipFactor,4):RadRotate(self.FGFoot.RotAngle),0,terrcheck,1,0,true);
				if groundray == true then
					...
					local usepos = self.FGLeg.Pos+Vector(2*self.FlipFactor,-self.kneedist):RadRotate(self.FGLeg.RotAngle)
					local getang = SceneMan:ShortestDistance(self.Pos, usepos, true).AbsRadAngle
					self:AddAbsForce(Vector(addforce,addforcey):RadRotate(getang), self.Pos+Vector(2*self.FlipFactor,-10):RadRotate(self.RotAngle))
			...
			if MovableMan:ValidMO(self.BGLeg) and self.BGFoot.Pos.X > self.Pos.X and self.BGLeg.RotAngle < self.triggerang then
				local groundray = SceneMan:CastStrengthRay(self.BGFoot.Pos,Vector(3*self.FlipFactor,4):RadRotate(self.BGFoot.RotAngle),0,terrcheck,1,0,true);
				if groundray == true then
					...
					local addforce = self.pscale*xchange*usemass
					local addforcey = -1*self.pscale*usemass*self.FlipFactor
					local usepos = self.BGLeg.Pos+Vector(2*self.FlipFactor,-self.kneedist):RadRotate(self.BGLeg.RotAngle)
					local getang = SceneMan:ShortestDistance(self.Pos, usepos, true).AbsRadAngle
					self:AddAbsForce(Vector(addforce,addforcey):RadRotate(getang), self.Pos+Vector(2*self.FlipFactor,-10):RadRotate(self.RotAngle))	
