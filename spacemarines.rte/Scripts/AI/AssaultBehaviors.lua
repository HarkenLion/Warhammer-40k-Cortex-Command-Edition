...
function AssaultBehaviors.LookForTargets(AI, Owner)
	local Origin
	local viewAng
	...
	local viewLen = SceneMan:ShortestDistance(Owner.EyePos, Owner.ViewPoint, false).Magnitude + FrameMan.PlayerScreenWidth * 0.55
...
function AssaultBehaviors.CheckEnemyLOS(AI, Owner)
	if not AI.Enemies then	-- add all enemy actors on our screen to a table and check LOS to them, one per frame
		...
		return AssaultBehaviors.LookForTargets(AI, Owner)	-- cast rays like normal actors occasionally
	else
			...
			if MovableMan:ValidMO(Enemy) then
				...
				if Enemy.ClassName == "ADoor" then
					...
					if Door and Door:IsAttached() then
						...
						return AssaultBehaviors.LookForTargets(AI, Owner)		-- this door is destroyed, cast rays like normal actors
					end
				else
...
function AssaultBehaviors.CalculateThreatLevel(MO, Owner)
...
function AssaultBehaviors.ProcessAlarmEvent(AI, Owner)
	AI.AlarmPos = nil
	...
	for Event in MovableMan.AlarmEvents do
		if Event.Team ~= Owner.Team then	-- caused by some other team's activites - alarming!
			loudness = 30 + FrameMan.PlayerScreenWidth * 0.5 * Owner.Perceptiveness * 2 * (Event.Range / 500)	-- adjust the audiable range to the screen resolution
			AlarmVec = SceneMan:ShortestDistance(Owner.EyePos, Event.ScenePos, false)	-- see how far away the alarm situation is
			if AlarmVec.Largest < loudness and not AI.Target then	-- only react if the alarm is within hearing range
			...
			elseif not AI.flying and Event.Range > 600 and PosRand() < 0.6 and Owner.FirearmIsReady then
				...
				if (AlarmVec.X < 0 and Owner.HFlipped) or (AlarmVec.X > 0 and not Owner.HFlipped) then
					if AlarmVec.Largest < FrameMan.PlayerScreenWidth * 1.8 and
						(not AI.LastAlarmVec or (AI.LastAlarmVec-AlarmVec).Largest > 20)
					then
						-- check LOS
						...
						if ID < rte.NoMOID then
							...
							if FoundMO and FoundMO:GetController():IsState(Controller.WEAPON_FIRE) and FoundMO.Vel.Largest < 20 then
...
function AssaultBehaviors.GetGrenadeAngle(AimPoint, TargetVel, StartPos, muzVel)
...
function AssaultBehaviors.EquipPrimaryWeapon(AI, Owner)
	if Owner.EquippedItem then
		if Owner.InventorySize > 0 and (not AI.PlayerPreferredHD or Owner.EquippedItem.PresetName ~= AI.PlayerPreferredHD) and
			not Owner.EquippedItem:HasObjectInGroup("Weapons - Primary") and Owner:HasObjectInGroup("Weapons - Primary")
		then
...
function AssaultBehaviors.Sentry(AI, Owner)
	local sweepUp = true
	...
	if AssaultBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
				if not AssaultBehaviors.EquipPrimaryWeapon(AI, Owner) then
...
function AssaultBehaviors.Patrol(AI, Owner)
...
function AssaultBehaviors.GoldDig(AI, Owner)	
...
function AssaultBehaviors.BrainSearch(AI, Owner)
	if AssaultBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
...
function AssaultBehaviors.WeaponSearch(AI, Owner)
	...
	if #Devices > 0 then
		...
		Owner:ClearAIWaypoints()
		table.sort(DevicesToPickUp, function(A,B) return A.range < B.range end)	-- sort the items in order of distance
		coroutine.yield()	-- wait until next frame
...
function AssaultBehaviors.ToolSearch(AI, Owner)
	local range, minDist, HD
	local Devices = {}
...
function AssaultBehaviors.GoToWpt(AI, Owner)
	...
	while true do
		...
		if Owner.MOMoveTarget then	-- make the last waypoint marker stick to the MO we are following
			if MovableMan:ValidMO(Owner.MOMoveTarget) then
				...
				if Owner.Team == Owner.MOMoveTarget.Team and not AI.flying then	-- we are following an ally, stop when close
					while SceneMan:ShortestDistance(Owner.Pos, Owner.MOMoveTarget.Pos, false).Magnitude < Owner.Radius+Owner.MOMoveTarget.Radius+10 do
						coroutine.yield()	-- wait until next frame
		...
		if not AI.flying and UpdatePathTimer:IsPastSimTimeLimit() then
			...
			if Waypoint and AI.BlockingActor then
				if MovableMan:IsActor(AI.BlockingActor) then
					...
					if (Owner.Pos.X > AI.BlockingActor.Pos.X and CurrDist.X < Owner.Pos.X) or
						(Owner.Pos.X < AI.BlockingActor.Pos.X and CurrDist.X > Owner.Pos.X) or
						SceneMan:ShortestDistance(Owner.Pos, AI.BlockingActor.Pos, false).Magnitude > Owner.Diameter + AI.BlockingActor.Diameter
					then
		...
		elseif WptList then	-- we have a list of waypoints, folow it
			if #WptList < 1 and not Waypoint then	-- arrived
				...
				if not Waypoint then	-- get the next waypoint in the list
					...
					if Owner.MOMoveTarget and MovableMan:ValidMO(Owner.MOMoveTarget) then
						if SceneMan:ShortestDistance(Owner:GetLastAIWaypoint(), Owner.MOMoveTarget.Pos, false).Largest > 80 then	-- the target has moved
							WptList = nil
							Waypoint = nil
						else
							UpdatePathTimer:Reset()
					...
					if WptList then						
						...
						if Dist.Y < -25 and math.abs(Dist.X) < 30 then	-- avoid any corners if the next waypoint is above us
							...
							if 0 <= SceneMan:CastObstacleRay(Owner.Pos, Dist, Vector(), Vector(), Owner.ID, Owner.IgnoresWhichTeam, rte.grassID, 4) then
							...
							if Waypoint.Type ~= "air" then
								...
								if (Owner.AIMode == Actor.AIMODE_GOLDDIG or Waypoint.Type ~= "drop") and Owner:EquipDiggingTool(false) then
									...
									if AI.teamBlockState ~= Actor.BLOCKED and SceneMan:CastStrengthRay(PrevWptPos, PathSegRay, 4, Free, 2, rte.doorID, true) then
										if SceneMan:ShortestDistance(Owner.Pos, Free, false).Magnitude < Owner.Height*0.5 then	-- check that we're close enough to start digging
				...
				if Waypoint then
					...
					if digState ~= AHuman.NOTDIGGING then
							...
							if Owner.FirearmIsEmpty then	-- reload if it's empty
								...
								if AI.teamBlockState == Actor.BLOCKED then
									...
									then
										...
										if SceneMan:ShortestDistance(Owner.Pos, Waypoint.Pos, false).Magnitude < Owner.Height*0.25 then
						...
						elseif AI.refuel and Owner.Vel.Y < 5 then
							if Owner.JetTimeLeft > Owner.JetTimeTotal * 0.95 or
								(AI.flying and Owner.Vel.Y < -3 and Owner.JetTimeLeft > 0.5)
							then
								AI.refuel = false
							else
						...
						if Waypoint and not AI.refuel then	-- move towards the waypoint
							...
							if math.abs(Owner.RotAngle) < 0.5 and Owner.Vel.Y > -10 and
								(change < -1 or Owner.Vel.Y > 10)
							then
								if not AI.jump then
									AI.jump = true
							...
							if not AI.jump and AI.flying and Waypoint.Type ~= "drop" and
								 Owner.Vel.Y > -5 and CurrDist.Y-Owner.Vel.Y*3 < 20	--TODO: use Ychange here?
							then
			...
			if Owner.AIMode == Actor.AIMODE_GOLDDIG and not AI.Target and not SceneMan:CastStrengthRay(Owner.Pos, Trace, 105, Vector(), 2, 0, true) then
				...
				for _, WptPos in pairs(PathDump) do
					...
					if SceneMan:ShortestDistance(Origin, WptPos, false).Magnitude > 100 or	-- skip any waypoint too close to the previous one
				...
				while #TmpWpts > 0 do
					...
					if Lower(NextWpt, StartWpt, 30) then	-- scan for sharp drops	TODO: check the slope instead?
						...
						for _, LZ in pairs(GapList) do
							...
							if -1 == SceneMan:CastObstacleRay(StartWpt.Pos, Trace, Vector(), Vector(), Owner.ID, Owner.IgnoresWhichTeam, rte.grassID, 4) then							
		...
		if AI.BlockingActor then
			if not MovableMan:IsActor(AI.BlockingActor) or
				SceneMan:ShortestDistance(Owner.Pos, AI.BlockingActor.Pos, false).Magnitude > (Owner.Diameter + AI.BlockingActor.Diameter)*1.2
			then
				AI.BlockingActor = nil
				AI.teamBlockState = Actor.NOTBLOCKED
			...
			elseif AI.teamBlockState == Actor.NOTBLOCKED and Waypoint then
				if (Waypoint.Pos.X > Owner.Pos.X and AI.BlockingActor.Pos.X > Owner.Pos.X) or
					 (Waypoint.Pos.X < Owner.Pos.X and AI.BlockingActor.Pos.X < Owner.Pos.X)
				then
					AI.teamBlockState = Actor.BLOCKED
					if Owner.AIMode == Actor.AIMODE_BRAINHUNT and Owner.AIMode == AI.BlockingActor.AIMode and
						(not AI.BlockingActor.MOMoveTarget or AI.BlockingActor.MOMoveTarget.ID ~= Owner.ID)
...
function AssaultBehaviors.GoProne(AI, Owner, TargetPos, targetID)
	
   if MovableMan:IsActor(AI.Target) then
...
function AssaultBehaviors.GetProjectileData(Owner)
	...
	if Round.IsEmpty then	-- set default values if there is no particle
		...
		if Projectile.ClassName == "MOPixel" then
			...
			PrjDat.pen = 0.5 * Projectile.Mass * Projectile.Sharpness * PrjDat.vel * math.max((Weapon.RateOfFire / 240), 1)
...
function AssaultBehaviors.ShootTarget(AI, Owner)
	if not MovableMan:IsActor(AI.Target) then
		Owner.NextBehavior = coroutine.create(AssaultBehaviors.AttackTarget)
		return true
	end

	
	AI.canHitTarget = false
	AI.TargetLostTimer:SetSimTimeLimitMS(400)
	
	local LOSTimer = Timer()
	LOSTimer:SetSimTimeLimitMS(170)
	
	local ShootTimer = Timer()
	local shootDelay = RangeRand(400, 600)
	local AimPoint = AI.Target.Pos + AI.TargetOffset
	if not AI.flying and AI.Target.Vel.Largest < 4 and AssaultBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID) then
	...
	while true do
		...
		if Owner.FirearmIsReady then
			...
			if not PrjDat or PrjDat.MagazineName ~= Weapon.Magazine.PresetName then
				PrjDat = AssaultBehaviors.GetProjectileData(Owner)
				...
				if checkAim then
					...
					if range < 100 then	-- within 5m
						...
						if PrjDat.exp and Owner.InventorySize > 0 then	-- we have more things in the inventory
							if Owner:EquipDiggingTool(false) then
								...
								break
							elseif canSwitchWeapon > 0 and Owner:HasObjectInGroup("Weapons - Primary") or Owner:HasObjectInGroup("Weapons - Secondary") then
					...
					elseif range < PrjDat.rng then
						...
						aimTarget = AssaultBehaviors.GetAngleToHit(PrjDat, Dist)
						if aimTarget then
							AI.canHitTarget = true
						else
							AI.canHitTarget = false
							...
							if canSwitchWeapon > 0 and (Owner:HasObjectInGroup("Weapons - Primary") or Owner:HasObjectInGroup("Weapons - Secondary")) then
								canSwitchWeapon = canSwitchWeapon - 1
								AI.Ctrl:SetState(Controller.WEAPON_CHANGE_NEXT, true)
								PrjDat = nil	-- get the ammo info from the new weapon the next update
...
function AssaultBehaviors.ThrowTarget(AI, Owner)
	local ThrowTimer = Timer()
	local aimTime = RangeRand(500, 950)
	local scan = 0
		...
		if scan < 1 then
			...
			if LOS then	-- don't sharp aim until LOS has been confirmed
				if Owner.ThrowableIsReady then
					aim = AssaultBehaviors.GetGrenadeAngle(AimPoint, AI.Target.Vel/RangeRand(0.5, 1.75), ToHeldDevice(Owner.EquippedItem).MuzzlePos, 18)
					if aim then
...
function AssaultBehaviors.AttackTarget(AI, Owner)
	if not AI.Target or not MovableMan:IsActor(AI.Target) then
		return true
	end
	
	AI.TargetLostTimer:SetSimTimeLimitMS(850)
...
function AssaultBehaviors.MoveAroundActor(AI, Owner)
...
function AssaultBehaviors.GetAngleToHit(PrjDat, Dist)
...
function AssaultBehaviors.ShootArea(AI, Owner)
	...
	if not AI.flying and AI.UnseenTarget.Vel.Largest < 12 and AssaultBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID) then
		aimTime = aimTime + 500
	end
...
function AssaultBehaviors.FaceAlarm(AI, Owner)
...
local Proxy = {}
local Mt = {
	__index = AssaultBehaviors,
	__newindex = function(Table, k, v)
		error("The AssaultBehaviors table is read-only.", 2)
	end
}
setmetatable(Proxy, Mt)
AssaultBehaviors = Proxy
