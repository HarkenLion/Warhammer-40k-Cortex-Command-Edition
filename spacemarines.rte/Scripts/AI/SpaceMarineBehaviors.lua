...
SpaceMarineBehaviors = {}

function SpaceMarineBehaviors.LookForTargets(AI, Owner)
	local Origin
	local viewAng
...
function SpaceMarineBehaviors.CheckEnemyLOS(AI, Owner)
	if not AI.Enemies then	-- add all enemy actors on our screen to a table and check LOS to them, one per frame
		...
		return SpaceMarineBehaviors.LookForTargets(AI, Owner)	-- cast rays like normal actors occasionally
	else
			...
			if MovableMan:ValidMO(Enemy) then
				...
				if Enemy.ClassName == "ADoor" then
					...
					if Door and Door:IsAttached() then
						...
						return SpaceMarineBehaviors.LookForTargets(AI, Owner)		-- this door is destroyed, cast rays like normal actors
					end
				else
...
function SpaceMarineBehaviors.CalculateThreatLevel(MO, Owner)
...
function SpaceMarineBehaviors.ProcessAlarmEvent(AI, Owner)
	AI.AlarmPos = nil
	...
	for Event in MovableMan.AlarmEvents do
		if Event.Team ~= Owner.Team then	-- caused by some other team's activites - alarming!
			loudness = 30 + FrameMan.PlayerScreenWidth * 0.75 * Owner.Perceptiveness * (Event.Range / 500)	-- adjust the audiable range to the screen resolution
...
function SpaceMarineBehaviors.GetGrenadeAngle(AimPoint, TargetVel, StartPos, muzVel)
...
function SpaceMarineBehaviors.EquipPrimaryWeapon(AI, Owner)
	if Owner.EquippedItem then
...
function SpaceMarineBehaviors.Sentry(AI, Owner)
	local sweepUp = true
	...
	if SpaceMarineBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
				if not SpaceMarineBehaviors.EquipPrimaryWeapon(AI, Owner) then
...
function SpaceMarineBehaviors.Patrol(AI, Owner)
...
function SpaceMarineBehaviors.GoldDig(AI, Owner)	
...
function SpaceMarineBehaviors.BrainSearch(AI, Owner)
	if SpaceMarineBehaviors.EquipPrimaryWeapon(AI, Owner) then
		local EquipTimer = Timer()
		while true do
			if EquipTimer:IsPastSimMS(500) then
...
function SpaceMarineBehaviors.WeaponSearch(AI, Owner)
...
function SpaceMarineBehaviors.ToolSearch(AI, Owner)
	local range, minDist, HD
	local Devices = {}
...
function SpaceMarineBehaviors.GoToWpt(AI, Owner)
...
function SpaceMarineBehaviors.GoProne(AI, Owner, TargetPos, targetID)
...
function SpaceMarineBehaviors.GetProjectileData(Owner)
...
function SpaceMarineBehaviors.ShootTarget(AI, Owner)
	...
	local shootDelay = RangeRand(200, 400)
	local AimPoint = AI.Target.Pos + AI.TargetOffset
	if not AI.flying and AI.Target.Vel.Largest < 4 and SpaceMarineBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID) then
	...
	local ErrorOffset = Vector(RangeRand(40, 80), 0):RadRotate(RangeRand(0, 4))
	...
	while true do
		...
		if Owner.FirearmIsReady then
			...
			if not PrjDat or PrjDat.MagazineName ~= Weapon.Magazine.PresetName then
				PrjDat = SpaceMarineBehaviors.GetProjectileData(Owner)
				...
				if range < 60 then
					AI.TargetOffset = AI.TargetOffset * 0.75
				end
				...
				if checkAim then
					...
					elseif range < PrjDat.rng then
						...
						aimTarget = SpaceMarineBehaviors.GetAngleToHit(PrjDat, Dist)
						if aimTarget then
							AI.canHitTarget = true
						else
							AI.canHitTarget = false
...
function SpaceMarineBehaviors.ThrowTarget(AI, Owner)
		...
		if scan < 1 then
			...
			if LOS then	-- don't sharp aim until LOS has been confirmed
				if Owner.ThrowableIsReady then
					aim = SpaceMarineBehaviors.GetGrenadeAngle(AimPoint, AI.Target.Vel/RangeRand(0.5, 1.75), ToHeldDevice(Owner.EquippedItem).MuzzlePos, 18)
					if aim then
...
function SpaceMarineBehaviors.AttackTarget(AI, Owner)
...
function SpaceMarineBehaviors.MoveAroundActor(AI, Owner)
...
function SpaceMarineBehaviors.GetAngleToHit(PrjDat, Dist)
...
function SpaceMarineBehaviors.ShootArea(AI, Owner)
	...
	if not AI.flying and AI.UnseenTarget.Vel.Largest < 12 and SpaceMarineBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID) then
		aimTime = aimTime + 200
	end
...
function SpaceMarineBehaviors.FaceAlarm(AI, Owner)
...
local Proxy = {}
local Mt = {
	__index = SpaceMarineBehaviors,
	__newindex = function(Table, k, v)
		error("The SpaceMarineBehaviors table is read-only.", 2)
	end
}
setmetatable(Proxy, Mt)
SpaceMarineBehaviors = Proxy
