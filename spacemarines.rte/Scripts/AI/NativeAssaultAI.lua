...
dofile("spacemarines.rte/Scripts/AI/AssaultBehaviors.lua")
require("AI/PID")

NativeAssaultAI = {}

function NativeAssaultAI:Create(Owner)
	local Members = {}
...
function NativeAssaultAI:Update(Owner)
	...
	if self.isBrain then
		FoundMO, HitPoint = AssaultBehaviors.CheckEnemyLOS(self, Owner)
	else
		FoundMO, HitPoint = AssaultBehaviors.LookForTargets(self, Owner)
	end
			...
			if FoundMO.Team == Owner.Team then	-- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if AssaultBehaviors.CalculateThreatLevel(FoundMO, Owner) > AssaultBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5 then
	...
	if not self.Target and not self.UnseenTarget then
		if self.AlarmTimer:IsPastSimTimeLimit() and AssaultBehaviors.ProcessAlarmEvent(self, Owner) then
...
-- functions that create behaviors. the default behaviors are stored in the AssaultBehaviors table. store your custom behaviors in a table to avoid name conflicts between mods.
function NativeAssaultAI:CreateSentryBehavior(Owner)
	...
	self.NextBehavior = coroutine.create(AssaultBehaviors.Sentry)	-- replace "AssaultBehaviors.Sentry" with the function name of your own sentry behavior
	self.NextCleanup = nil
	self.NextBehaviorName = "Sentry"
end

function NativeAssaultAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(AssaultBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeAssaultAI:CreateGoldDigBehavior(Owner)
	self.NextBehavior = coroutine.create(AssaultBehaviors.GoldDig)
	self.NextCleanup = nil
	self.NextBehaviorName = "GoldDig"
end

function NativeAssaultAI:CreateBrainSearchBehavior(Owner)
	self.NextBehavior = coroutine.create(AssaultBehaviors.BrainSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "BrainSearch"
end

function NativeAssaultAI:CreateGetToolBehavior(Owner)
	self.NextBehavior = coroutine.create(AssaultBehaviors.ToolSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "ToolSearch"
end

function NativeAssaultAI:CreateGetWeaponBehavior(Owner)
	self.NextBehavior = coroutine.create(AssaultBehaviors.WeaponSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "WeaponSearch"
end

function NativeAssaultAI:CreateGoToBehavior(Owner)
	self.NextGoTo = coroutine.create(AssaultBehaviors.GoToWpt)
...
function NativeAssaultAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()
	
	if Owner:EquipDiggingTool(true) and SceneMan:ShortestDistance(Owner.Pos, self.Target.Pos, false).Magnitude < 250 then
		self.NextBehavior = coroutine.create(AssaultBehaviors.AttackTarget)
		self.NextBehaviorName = "AttackTarget"
	elseif Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(AssaultBehaviors.ShootTarget)
		self.NextBehaviorName = "ShootTarget"
	elseif Owner:EquipThrowable(true) then
		self.NextBehavior = coroutine.create(AssaultBehaviors.ThrowTarget)
...
function NativeAssaultAI:CreateHtHBehavior(Owner)
...
function NativeAssaultAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(AssaultBehaviors.ShootArea)
...
function NativeAssaultAI:CreateMoveAroundBehavior(Owner)
	self.NextGoTo = coroutine.create(AssaultBehaviors.MoveAroundActor)
...
function NativeAssaultAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(AssaultBehaviors.FaceAlarm)
