...
function Create(self)
	...
	function self.drawcape()
		...
		PrimitiveMan:DrawPolygonFillPrimitive(startpos, 11, drawverts);
	end
...
--HANDLE CAPE
	...
	while i > -1 do
		if i == 0 or i == 11 then
			...
			local usepos = self.Pos + Vector((2-usenum)*self.FlipFactor,-11):RadRotate(self.RotAngle)
			self.apx[i] = usepos.X
			self.apy[i] = usepos.Y
		...
		while j < 2 do
			...
			local cablelength = 7;
				...
				if i == 5 then cablelength = 12 end
