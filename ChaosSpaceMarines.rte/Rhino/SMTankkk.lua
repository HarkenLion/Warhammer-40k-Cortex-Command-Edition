...
function Create(self)
	...
	for attachable in self.Attachables do
		if
			attachable.PresetName == "Rhino Tank Tread"
			or attachable.PresetName == "Predator Tank Tread BL"
			or attachable.PresetName == "Rhino Tank Tread IF"
		then
	...
	if MovableMan:ValidMO(self) then
		...
		if self.Vel.Y > -1 then
			...
			local groundray = SceneMan:CastStrengthRay(
				self.Pos + Vector(-28 * self.FlipFactor, 7):RadRotate(self.RotAngle),
				Vector(55 * self.FlipFactor, 7):RadRotate(self.RotAngle),
				0,
				terrcheck,
				1,
				0,
