function Create(self)
	...
	self.critsMax = 25 -- max amount of critical hits per swing
	...
	if parent and IsAHuman(parent) then
		...
		elseif self.swing == true then
			...
			if self.connect == true or self.num > math.pi * 1.5 then
				...
				local part = CreateMOPixel("Particle Powersword H")
				part.Pos = self.MuzzlePos

				part.Vel = parent.Vel + Vector((100 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)

				part.Mass = 0.1 * RangeRand(0.75, 1.25)
				part.Sharpness = 850 * RangeRand(0.75, 1.25)
				...
				if moid ~= 255 and moid ~= 0 then
					...
					if hitMO and (MovableMan:IsActor(root)) then
						...
						local soundfx = CreateAEmitter("Lightning ImpactH")
