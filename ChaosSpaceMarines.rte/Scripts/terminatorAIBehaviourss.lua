terminatorAIBehaviours = {}
terminatorAIBehaviours = {}

-- function terminatorAIBehaviours.createSoundEffect(self, effectName, variations)
...
function terminatorAIBehaviours.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						terminatorAIBehaviours.createEmotion(self, emotion, priority, 0, canOverridePriority)
					end
...
function terminatorAIBehaviours.createEmotion(self, emotion, priority, duration, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false
	end
	local usingPriority
...
function terminatorAIBehaviours.handleMovement(self)
...
function terminatorAIBehaviours.handleHealth(self)
	...
	if healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured then
		...
		if not self.FGArm and (self.FGArmLost ~= true) then
			...
			self.FGArmLost = true
			terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedHigh, 5, 4)
		end
		...
		elseif wasLightlyInjured then
			terminatorAIBehaviours.createEmotion(self, 2, 1, 500)
		...
		if wasInjured or wasHeavilyInjured and self.Head then
			if self.Health > 0 then
				terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Pain, 2, 2)
			else
				terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Death, 10, 4)
			end
		end
	end
end

function terminatorAIBehaviours.handleDying(self)
	if self.Head then
...
function terminatorAIBehaviours.handleSuppression(self)
	...
	if blinkTimerReady and not self.Suppressed and self.Head then
		if self.Head.Frame == self.baseHeadFrame then
			terminatorAIBehaviours.createEmotion(self, 1, 0, 100)
		...
		if self.Suppression > 25 then
			if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) then
				...
				elseif self.Suppression > 80 then
					terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 4, 4)
					self.suppressedVoicelineTimer:Reset()
					self.suppressionUpdates = 0
				end
				if self.Suppressed == false then -- initial voiceline
					if self.Suppression > 55 then
						...
						terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedLow, 3, 2)
					end
					self.suppressedVoicelineTimer:Reset()
				end
			end
...
function terminatorAIBehaviours.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
	...
	if (not self:IsPlayerControlled()) and self.AI.Target and IsAHuman(self.AI.Target) then
		...
		if self.spotAllowed ~= false then
			if self.LastTargetID == -1 then
				...
				if
					not self.AI.Target:NumberValueExists("Heat Enemy Spotted Age") -- If no timer exists
					or self.AI.Target:GetNumberValue("Heat Enemy Spotted Age") < (self.AI.Target.Age - self.AI.Target:GetNumberValue(
						"Heat Enemy Spotted Delay"
					)) -- If the timer runs out of time limit
					or math.random(0, 100) < self.spotIgnoreDelayChance -- Small chance to ignore timers, to spice things up
				then
					-- Setup the delay timer
					self.AI.Target:SetNumberValue("Heat Enemy Spotted Age", self.AI.Target.Age)
					self.AI.Target:SetNumberValue(
						"Heat Enemy Spotted Delay",
						math.random(self.spotDelayMin, self.spotDelayMax)
					)

					self.spotAllowed = false

					terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3)
				end
			else
...
function terminatorAIBehaviours.handleDying(self)
	if self.Head then
		if self.deathSoundPlayed ~= true then
			self.deathSoundPlayed = true
			terminatorAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Death, 10)
		end
	end
end
