			...
			if FoundMO.Team == Owner.Team then -- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if
							AssaultBehaviors.CalculateThreatLevel(FoundMO, Owner)
							> AssaultBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
						then
...
function NativeAssaultAI:CreateAttackBehavior(Owner)
	...
	if
		...
		and SceneMan:ShortestDistance(Owner.Pos, self.Target.Pos, false).Magnitude < 250
	then
