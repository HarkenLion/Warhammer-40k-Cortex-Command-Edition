...
function SpaceMarineBehaviors.ShootTarget(AI, Owner)
	...
	if
		not AI.flying
		and AI.Target.Vel.Largest < 4
		and SpaceMarineBehaviors.GoProne(AI, Owner, AimPoint, AI.Target.ID)
	then
		...
		if scan < 1 then
			...
			if LOS then -- don't sharp aim until LOS has been confirmed
				if Owner.ThrowableIsReady then
					aim = SpaceMarineBehaviors.GetGrenadeAngle(
						AimPoint,
...
function SpaceMarineBehaviors.ShootArea(AI, Owner)
	...
	if
		...
		and SpaceMarineBehaviors.GoProne(AI, Owner, AI.UnseenTarget.Pos, AI.UnseenTarget.ID)
	then
		aimTime = aimTime + 200
	end
