dofile("spacemarines.rte/Scripts/AI/SpaceMarineBehaviors.lua")
require("AI/PID")

NativeSpaceMarineAI = {}

function NativeSpaceMarineAI:Create(Owner)
	local Members = {}
...
function NativeSpaceMarineAI:Update(Owner)
	...
	if self.isBrain then
		FoundMO, HitPoint = SpaceMarineBehaviors.CheckEnemyLOS(self, Owner)
	else
		FoundMO, HitPoint = SpaceMarineBehaviors.LookForTargets(self, Owner)
	end
			...
			if FoundMO.Team == Owner.Team then -- found an ally
				...
				if FoundMO then
					if self.Target then
						...
						if
							SpaceMarineBehaviors.CalculateThreatLevel(FoundMO, Owner)
							> SpaceMarineBehaviors.CalculateThreatLevel(self.Target, Owner) + 0.5
						then
	...
	if not self.Target and not self.UnseenTarget then
		if self.AlarmTimer:IsPastSimTimeLimit() and SpaceMarineBehaviors.ProcessAlarmEvent(self, Owner) then
...
-- functions that create behaviors. the default behaviors are stored in the SpaceMarineBehaviors table. store your custom behaviors in a table to avoid name conflicts between mods.
function NativeSpaceMarineAI:CreateSentryBehavior(Owner)
	...
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.Sentry) -- replace "SpaceMarineBehaviors.Sentry" with the function name of your own sentry behavior
	self.NextCleanup = nil
	self.NextBehaviorName = "Sentry"
end

function NativeSpaceMarineAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeSpaceMarineAI:CreateGoldDigBehavior(Owner)
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.GoldDig)
	self.NextCleanup = nil
	self.NextBehaviorName = "GoldDig"
end

function NativeSpaceMarineAI:CreateBrainSearchBehavior(Owner)
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.BrainSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "BrainSearch"
end

function NativeSpaceMarineAI:CreateGetToolBehavior(Owner)
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.ToolSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "ToolSearch"
end

function NativeSpaceMarineAI:CreateGetWeaponBehavior(Owner)
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.WeaponSearch)
	self.NextCleanup = nil
	self.NextBehaviorName = "WeaponSearch"
end

function NativeSpaceMarineAI:CreateGoToBehavior(Owner)
	self.NextGoTo = coroutine.create(SpaceMarineBehaviors.GoToWpt)
...
function NativeSpaceMarineAI:CreateAttackBehavior(Owner)
	self.ReloadTimer:Reset()
	self.TargetLostTimer:Reset()

	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(SpaceMarineBehaviors.ShootTarget)
		self.NextBehaviorName = "ShootTarget"
	elseif Owner:EquipThrowable(true) then
		self.NextBehavior = coroutine.create(SpaceMarineBehaviors.ThrowTarget)
		self.NextBehaviorName = "ThrowTarget"
	elseif
		Owner:EquipDiggingTool(true)
		and SceneMan:ShortestDistance(Owner.Pos, self.Target.Pos, false).Magnitude < 150
	then
		self.NextBehavior = coroutine.create(SpaceMarineBehaviors.AttackTarget)
...
function NativeSpaceMarineAI:CreateHtHBehavior(Owner)
...
function NativeSpaceMarineAI:CreateSuppressBehavior(Owner)
	if Owner:EquipFirearm(true) then
		self.NextBehavior = coroutine.create(SpaceMarineBehaviors.ShootArea)
...
function NativeSpaceMarineAI:CreateMoveAroundBehavior(Owner)
	self.NextGoTo = coroutine.create(SpaceMarineBehaviors.MoveAroundActor)
...
function NativeSpaceMarineAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(SpaceMarineBehaviors.FaceAlarm)
