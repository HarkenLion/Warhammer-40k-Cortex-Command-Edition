...
function Create(self)
	...
	if math.random(0, 1) == 0 then
		self.voiceSounds = {
			Death = CreateSoundContainer("VO Death VariantOne", "chaosspacemarines.rte"),
			Spot = CreateSoundContainer("VO Spot VariantOne", "chaosspacemarines.rte"),
		}
	else
		self.voiceSounds = {
			Death = CreateSoundContainer("VO Death VariantOne", "chaosspacemarines.rte"),
...
function Update(self)
	...
	if self:IsDead() ~= true then
		ChaosMarineAIBehaviours.handleHealth(self)

		ChaosMarineAIBehaviours.handleAITargetLogic(self)
	else
		ChaosMarineAIBehaviours.handleDying(self)
	end
end
-- End modded code --
