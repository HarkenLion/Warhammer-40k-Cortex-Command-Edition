function Create(self)
	...
	local glow = CreateMOPixel("Warp Lightning Glow 2")
	glow.Pos = self.Pos
	MovableMan:AddParticle(glow)

	self.pointCount = math.floor(500 * RangeRand(0.9, 1.1) + 0.5) -- Number of points
	self.spiralScale = 15 * RangeRand(0.85, 1.15) -- Size of the spiral
	self.skipPoints = 5
	...
	self.targetPos = self.Pos
		+ Vector(self.spiralScale * 13 * RangeRand(0.8, 1.2), 0):RadRotate(self.RotAngle + RangeRand(-1, 1) * 0.35)
	local hit = false
	...
	local midpoint = Vector(self.spiralScale * 3 * RangeRand(0.8, 1.2), 0):RadRotate(self.RotAngle)
		+ Vector(RangeRand(-1, 1), RangeRand(-1, 1)) * 20
	for i = 1, maxi do
		local fac = i / maxi
		local facRev = 1 - fac
		...
		local pos = self.Pos
			+ (p1 * math.pow(1 - fac, 2) + p2 * 2 * (1 - fac) * fac + p3 * math.pow(fac, 2))
			+ Vector(RangeRand(-1, 1), RangeRand(-1, 1)) * 3
		local glow = CreateMOPixel("Warp Lightning Glow " .. math.random(1, 3))
		glow.Pos = pos
		MovableMan:AddParticle(glow)
		local glow = CreateMOPixel("Tesla Rifle Glow 10")
		glow.Pos = pos
		MovableMan:AddParticle(glow) 
		local glow = CreateMOPixel("Tesla Rifle Glow 30 Short " .. math.random(3))
		glow.Pos = pos
		MovableMan:AddParticle(glow)
		...
		glow.Pos = lastPos + SceneMan:ShortestDistance(lastPos, pos, SceneMan.SceneWrapsX) * 0.5
		MovableMan:AddParticle(glow)
		--local glow = CreateMOSParticle("Warp Explosion Smoke Small " .. math.random(1, 1))
		--glow.Pos = lastPos + SceneMan:ShortestDistance(lastPos, pos, SceneMan.SceneWrapsX) * 0.5
		----MovableMan:AddParticle(glow)

		PrimitiveMan:DrawLinePrimitive(lastPos, pos, 211)
	...
	if hitMO ~= nil then
		...
		if IsMOSRotating(hitMO) then
			hitMO = ToMOSRotating(hitMO)
		end
		if IsAttachable(hitMO) then
			hitMO = ToAttachable(hitMO)
		end
		...
		if actor and actor.ClassName ~= "ADoor" then
			...
			if IsACDropShip(actor) then
				...
				for _, name in pairs{ "RightEngine", "LeftEngine", "RightThruster", "LeftThruster" } do
					if dropship[name] and math.random(0, 2) == 0 then
						dropship[name]:EnableEmission(false);
					end
				end
			end

			local wound = CreateAEmitter(
				actor:GetEntryWoundPresetName(),
				PresetMan:GetDataModule(actor.ModuleID).FileName
			)
	...
	if hit then
		...
		local spark = CreateMOPixel("Spark Warp Lightning 1")
