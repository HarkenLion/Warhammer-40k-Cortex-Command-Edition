...
function Create(self)
	...
	local glow = CreateMOPixel("Lighting Warp Glow 2")
	glow.Pos = self.Pos
	MovableMan:AddParticle(glow)

	--self.lightingHitSound = CreateSoundContainer("Warp Hit Taser", "chaosspacemarines.rte");
	...
	for i = 1, maxi do
		...
		local glow = CreateMOPixel("Lighting Warp Glow " .. math.random(1, 3))
		glow.Pos = pos
		MovableMan:AddParticle(glow)
		...
		PrimitiveMan:DrawLinePrimitive(lastPos, pos, 244)
	...
	if hit then
		if math.random(1, 7) < 2 then
			self.explosion = CreateAEmitter("chaosspacemarines.rte/Warp Hit Effect")
		...
		local spark = CreateMOPixel("Spark Warp Lighting 1")
