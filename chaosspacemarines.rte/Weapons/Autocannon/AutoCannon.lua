...
function Update(self)
	...
	if
		not self.currentlySpinningDown
		and not self:IsActivated()
		and not self:IsReloading()
		and self.ActiveSound:IsBeingPlayed()
	then
		self.spinDownTimer:Reset()
		self.currentlySpinningDown = true
		self.activationDelay = self.origActivationDelay
	elseif
		(self.currentlySpinningDown and self.spinDownTimer:IsPastSimMS(self.DeactivationDelay)) or self:IsReloading()
	then
		self.ActivationDelay = self.origActivationDelay
		self.currentlySpinningDown = false
