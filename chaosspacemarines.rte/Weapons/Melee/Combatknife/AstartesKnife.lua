function Create(self)
	...
	self.speed = 0.25 -- variable(not)
	self.equipSound = CreateSoundContainer("Astartes Knife Equip", "spacemarines.rte")
	self.swingSound = CreateSoundContainer("Astartes Knife Swing", "spacemarines.rte")
	self.bounceSound = CreateSoundContainer("Astartes Knife Bounce", "spacemarines.rte")
...
function Update(self)
	...
	if parent and IsAHuman(parent) then
		...
		local speed = self.speed * parent.Sharpness
		...
		if self.cooldown == true then
			...
			local recovertime = math.max(0, 155 - 25 * parent.Sharpness)
		...
		elseif self.swing == true then
			...
			if self.connect == true or self.num > math.pi * 1.5 then
				...
				local tracevec = parent.Vel + Vector((25 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
				local moid = SceneMan:CastMORay(self.MuzzlePos, tracevec, self.ID, self.Team, 0, false, 3)

				local part = CreateMOPixel("Particle Astartes Knife 1")
				part.Pos = self.MuzzlePos

				part.Vel = parent.Vel + Vector((85 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
				part.Mass = 0.25 * RangeRand(0.75, 1.25)
				part.Sharpness = 15 * RangeRand(0.75, 1.25)
				...
				if moid ~= 255 and moid ~= 0 then
					for i = 0, 2 do
						...
						part.Vel = parent.Vel + Vector((85 + velBonus) * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
						part.Mass = 0.3 * RangeRand(0.75, 1.25)
						part.Sharpness = 15 * RangeRand(0.75, 1.25)
						part.Team = self.Team
						part.IgnoresTeamHits = true
					...
					local tracevec = Vector(17 * self.FlipFactor, 0):RadRotate(rotAngle - 0.3)
					local ustren = 75 + parent.Sharpness * 5 --90 --100
					local hitvec = Vector(0, 0)
					local sray = SceneMan:CastStrengthRay(startvec, tracevec, ustren, hitvec, 1, 0, true)
					if sray == true then
		...
		if self.windup ~= true and self.swing ~= true then
			offset = Vector(11, 11):RadRotate(-0.25 + math.sin(self.num) * 1.4)
			rotAngle = rotAngle + 2.1 ---0.9
			self.Supportable = false
		else
			offset = Vector(18, 0):RadRotate(-0.25 + math.sin(self.num) * 1.2)
