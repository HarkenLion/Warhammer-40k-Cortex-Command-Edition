	...
	if speed > 50 then
		...
		if moid ~= 255 and moid ~= 0 then
			...
			if
				hitMO:IsHeldDevice()
				and hitMO.Mass > 5
				and hitMO.ClassName == "HDFirearm"
				and hitMO.PresetName ~= ""
			then
			...
			--SUNDER ARMOR
			if
				(hitMO.ClassName == "Attachable")
				and hitMO.Mass < 15
				and ((string.find(hitMO.PresetName, "Armor") ~= nil) or (string.find(hitMO.PresetName, "Plate") ~= nil))
			then
				hitMO:AddImpulseForce((checkVect / velFactor) * self.Mass, Vector(0, 0))
				hitMO.AngularVel = 0
			...
			--HEADSHOT DAMAGE
			if
				(string.find(hitMO.PresetName, "Head") ~= nil)
				or (string.find(hitMO.PresetName, "Helmet") ~= nil)
				or (string.find(hitMO.PresetName, "Hat") ~= nil)
			then
			...
			if
				(hitMO.ClassName == "Attachable")
				and hitMO.Mass < 45
				and ((string.find(hitMO.PresetName, "Arm") ~= nil) or (string.find(hitMO.PresetName, "Leg") ~= nil))
			then
